...
function configureRunnableSettings() {
  const modelPool: ModelPoolEntry[] = [
    ...
    { id: 'qwen', alias: 'Qwen', type: 'llm', provider: 'dashscope', baseUrl: 'https://dashscope.aliyuncs.com/compatible-mode/v1', apiKey: 'sk-test-secret', modelName: 'qwen3.5-omni-flash', supportsVision: true },
  ]
  useRainStore.setState({
    ...
    modelPool,
    capabilityRecords: [
      recordCapabilityCheck({ model: runtimeModelFromPoolEntry(modelPool[0]), role: 'asr', ok: true, message: 'ASR probe passed' }),
      recordCapabilityCheck({ model: runtimeModelFromPoolEntry(modelPool[1]), role: 'structuring', ok: true, message: 'Structuring probe passed' }),
    ],
...
describe('VideoListPage import recovery UI', () => {
  ...
  it('blocks the real page entry when required capability records are missing', async () => {
    const db = await getDb()
    await insertVideo(db, video('capability-video'))
    configureRunnableSettings()
    useRainStore.setState({ capabilityRecords: [] })

    render(<VideoListPage />)
    await screen.findByTestId('card-capability-video')
    ...
    expect(screen.getByRole('alert')).toHaveTextContent(/^ASR 模型“Whisper”不可用：/)
