...
| SU-03 | Partial | StudyInterface 以原生 `hidden` 对称保留 AI/NotesPanel；可见 AI wrapper 为列 flex，NoteItem 与 ChatInput 本地草稿不 remount | study-notes/layout/M08/M10 — Partial | 阻断：Strong + Desktop Evidence；生产 StudyInterface/Store/公开 `streamAiChat` callback Judge 现覆盖 Note memory 写入、初始 Notes fixture 的 AI 未发送草稿 hidden/a11y/Tab 隔离与恢复，以及流中 Tab 切换不 cleanup/不重复请求、hidden token/完成归原请求、切回连续回答、独立 request cleanup identity、第一条 stale callback 后对未污染第二回答的即时 DOM 断言与第二条停止后晚到 token 忽略；其余隐藏副作用与 Desktop Evidence 未闭合 | 禁用 jsdom/假截图替代 |
