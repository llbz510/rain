...
裁判：`model-capabilities.test.ts`、`model-pool.test.ts`、`preflight.test.ts`、`video-import-capability-gate.test.ts` 和对应角色的真实行为测试。完整真实 E2E 负责把一个兼容组合提升为 `Verified`；普通连接或文件存在检查不得产生 `Compatible` 或伪造 `Verified`。
