...
本节来自 2026-07-27 对当前设置页、Rust command 和历史模型管理规格的核对。三条 AC 均为 `Proposed`，在用户确认前只用于暴露决策和 Harness 缺口，不授权实现。
...
实现归属（提案）：新 Rust `whisper_model_download` module 负责 manifest、流式写入、验证、临时文件和原子替换；`commands.rs` 只解析应用目录并调用其公开 interface。

裁判（提案）：Rust module 使用本地 HTTP fixture 和临时目录，直接覆盖成功提交、重复下载、截断响应、哈希错误、写入/替换失败、旧文件保留和临时文件清理。测试不得访问真实 Hugging Face 或下载 GB 级模型。
...
实现归属（提案）：`whisper_model_download` module 持有按 model size 区分的下载 lease/cancellation token；Tauri 新增独立 `cancel_whisper_model_download` adapter，不能复用视频导入的 `cancel_import`。

裁判（提案）：Rust fake stream/recording writer 直接证明分块消费、进度单调、重复启动隔离、取消停止、取消清理和取消后重试。新增 command 会改变 M20 的精确 command 集合，实施前必须得到用户对 Harness Migration 的明确批准。
...
实现归属（提案）：设置页模型下载 workflow 负责订阅/释放事件和 UI 状态；表单只收集 model size 和展示 workflow 状态；Rust 是下载任务和文件状态的事实源。

裁判（提案）：新增非锁定设置页行为测试，通过生产 Tauri adapter/event interface 驱动真实表单，覆盖进度、取消、失败、重试、监听释放和成功后刷新模型列表。静态文案、按钮存在或直接修改 React state 不构成通过。
