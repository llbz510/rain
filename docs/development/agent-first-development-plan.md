...
本节的 P0–P5 是候选优先级标签，不是独立于总交付路线图的执行顺序。唯一权威顺序见 `docs/development/rain-project-delivery-plan.md`：M1 发布范围冻结和 M2 当前证据重放已经完成；受控 GPU artifact build 被用户暂停，取消 run 未产生 manifest、core/control artifact、build record 或 launcher。当前唯一方向是按 `launch-feature-audit.md` 的 blocker ledger 逐个关闭用户可见的 Confirmed Launch feature Slice；M3 Release Evidence、签名、许可和分发只有用户明确恢复后才重新排期。
...
当前原子工作只可选择一个 **Launch feature Slice**：从 `launch-feature-audit.md` 选择一个用户可见的 Confirmed AC 缺口，先在生产公开入口建立 RED，再以最小 GREEN 收口。受控 GPU artifact build、真实 NVIDIA Evidence、安装器、签名、许可、Release 与下载页均为 user-paused；取消 run 未生成或上传任何 manifest、core/control artifact、build record 或 launcher。除非用户明确恢复，不得 dispatch workflow、构建候选、运行 GPU/模型/LLM 或升级 `AC-RL-*`。每个 Slice 仍须按本计划执行 TDD、允许的本地验证、独立审查与状态交接；审计不修改 AC，也不让测试存在等同功能完成。
