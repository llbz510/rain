...
| DEC-PRD-011 | `M05-catalog-zone.md` | Confirmed AC | `AC-SU-01`、`AC-ST-03` | 随播目录采用章节与小节顶行、段落底行的横向结构 |
| DEC-PRD-012 | `M05-catalog-zone.md` | Confirmed AC | `AC-SU-02`、`AC-UX-05`、`AC-ST-03` | 目录进度实时填充，跨章节或小节切换使用约 200ms 横滑 |
| DEC-PRD-013 | `M05-catalog-zone.md` | Confirmed AC | `AC-SU-01` | 目录项不折叠缩略，使用横向滚动、自动居中和边缘渐隐 |
...
| DEC-PRD-022 | `M16-layout.md` | Confirmed AC | `AC-SU-03`、`AC-ST-08` | 右侧面板用 Tab 在 AI 助手与随记间切换 |
| DEC-PRD-023 | `M13-visual-design.md`、`M16-layout.md` | Confirmed AC | `AC-SU-04`、`AC-SU-05` | 布局比例可调并记忆，视频字幕与滚动转录稿分离 |
...
| DEC-PRD-053 | `M14-keyboard-shortcuts.md` | Confirmed AC | `AC-SU-07`、`AC-UX-06`；M1-S2 的更具体合同取代旧 M14 删除快捷键，节点删除/编辑保持 Post-release | Core 非输入态提供模式、播放、跳段、摘注和面板快捷键；`Del/Backspace` 禁用直到高级树删除另行确认 |
...
| DEC-PRD-057 | `M13-visual-design.md`、`M17-video-list.md` | Confirmed AC | `AC-VL-01`、`AC-VL-07`、`AC-LV-10` | ready 与非 ready 视频卡采用缩略图网格和不同信息层级 |
| DEC-PRD-058 | `M17-video-list.md` | Confirmed AC | `AC-VL-02` | 视频列表支持最近学习、导入时间、名称三种排序 |
| DEC-PRD-059 | `M01-positioning.md`、`M17-video-list.md` | Confirmed AC | `AC-VL-03` | v1 只按标题关键词搜索，不建设标签、筛选或跨视频正文检索 |
| DEC-PRD-060 | `M15-data-persistence.md`、`M17-video-list.md` | Confirmed AC | `AC-VL-05`、`AC-VL-06`、`AC-LV-13` | 删除视频以单事务级联业务数据、删除应用缩略图，并保留用户本地源文件 |
| DEC-PRD-061 | `M17-video-list.md` | Out-of-scope | v1 不提供学习数据或视频导出 | 暂不实现导出 |
| DEC-PRD-062 | `M17-video-list.md` | Confirmed AC | `AC-VL-02`、`AC-VL-03`、`AC-VL-04`、`AC-LV-10`、`AC-LV-19` | 列表提供导入、排序、搜索、空状态和非 ready 任务入口 |
| DEC-PRD-063 | `M13-visual-design.md` | Confirmed AC | `AC-UX-01` | 只提供暗色主题 |
| DEC-PRD-064 | `M13-visual-design.md` | Confirmed AC | `AC-UX-01` | 背景、面板、分隔和文字采用四档中性色阶 |
| DEC-PRD-065 | `M13-visual-design.md` | Confirmed AC | `AC-UX-01` | 不使用品牌强调色，控件依靠灰白、明暗和字重 |
| DEC-PRD-066 | `M13-visual-design.md` | Confirmed AC | `AC-UX-02` | 四种段落类型拥有全界面唯一的蓝、绿、橙、灰映射 |
| DEC-PRD-067 | `M13-visual-design.md` | Confirmed AC | `AC-UX-02` | 所有选中态以变暗表示 |
| DEC-PRD-068 | `M13-visual-design.md` | Confirmed AC | `AC-UX-02` | 当前播放变亮，选中与播放同现时叠加暗底和白亮条 |
| DEC-PRD-069 | `M13-visual-design.md` | Confirmed AC | `AC-UX-02` | 失败红、处理中黄、排队灰独立于强调色 |
| DEC-PRD-070 | `M13-visual-design.md` | Confirmed AC | `AC-UX-02` | 段落进度用类型色，容器用中性色，节点文字保持白色 |
| DEC-PRD-071 | `M13-visual-design.md` | Confirmed AC | `AC-UX-02` | 文本区类型胶囊保留类型色文字、淡底和细边 |
| DEC-PRD-072 | `M13-visual-design.md` | Out-of-scope | 本行的部分卡片细节已由 DEC-PRD-077 明确替代；当前以 077 为准 | 早期列表卡片视觉草案不再单独作为当前合同 |
| DEC-PRD-073 | `M13-visual-design.md` | Confirmed AC | `AC-UX-03` | 使用系统无衬线单族、等宽数字特性和 400、600、700 字重 |
| DEC-PRD-074 | `M13-visual-design.md` | Confirmed AC | `AC-UX-03` | 使用 18、16、14、13、12 五档字号 |
| DEC-PRD-075 | `M13-visual-design.md` | Confirmed AC | `AC-UX-03` | 正文、标题、次正文分别采用 1.7、1.3、1.5 行距 |
| DEC-PRD-076 | `M13-visual-design.md` | Confirmed AC | `AC-UX-04` | 统一间距、圆角、阴影和关键区域高度令牌 |
| DEC-PRD-077 | `M13-visual-design.md`、`M17-video-list.md` | Confirmed AC | `AC-VL-07` | 视频列表使用 240px 下限响应式网格、16:9 缩略图和规定信息布局 |
| DEC-PRD-078 | `M13-visual-design.md` | Confirmed AC | `AC-SU-05` | 视频字幕使用底部半透明框、16px 白字与原文逐句内容 |
| DEC-PRD-079 | `M13-visual-design.md` | Confirmed AC | `AC-SU-06`、`AC-ST-04` | 导图使用类型色节点、正交圆弧连线和限定缩放平移 |
| DEC-PRD-080 | `M13-visual-design.md` | Proposed | 文本助手核心受 `AC-ST-07` 控制；对话气泡、当前帧、芯片和输入手势没有统一 AC | 右侧助手采用气泡、快捷芯片、多行输入和当前帧提示 |
| DEC-PRD-081 | `M13-visual-design.md` | Confirmed AC | `AC-UX-05`、`AC-UX-06` | 动效统一为 120、200、320ms 并尊重系统减少动效设置 |
...
| DEC-PRD-092 | `M20-architecture.md` | Confirmed AC | `AC-AR-02` | 所有 LLM 请求由前端直接连接 OpenAI-compatible 接口 |
...
| DEC-PRD-096 | `M20-architecture.md` | Confirmed AC | `AC-AR-03`、`AC-ST-01`、`AC-ST-02` | 本地媒体和缩略图通过 convertFileSrc 与 asset protocol 播放 |
...
| DEC-PRD-099 | `M20-architecture.md` | Confirmed AC | `AC-AR-04`、`AC-LV-16`、`AC-ST-03`、`AC-ST-08` | Zustand 只保存 UI 会话态与当前视频缓存，持久业务事实留在 SQLite |

## 读取结论

- 72 条决策映射到当前 Confirmed AC；这只表示产品语义已经有正式去向，不表示实现或 Evidence 已完成。
- 23 条决策保留为 Post-release Proposed；其中大量行为可能已有局部实现或组件 Harness，但尚不能形成完整完成声明。
