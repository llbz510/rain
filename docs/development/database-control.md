...
7. `database-boundary.test.ts` 禁止生产模块直接导入 `database-adapter`、`database-checkpoints`、`database-import-state` 或 `database-schema`，并证明内存 adapter 不伪装支持 SQL。内部实现可以继续拆分，调用者仍只能看到稳定公共入口。
...
2. `In progress`：导入状态、检查点和原子合并。可判别的内部 adapter interface、独立检查点模块、状态转换和恢复判断已完成；ASR 保存、句子归属和最终合并仍在 `database.ts`，后续继续由 AC-LV-03 至 AC-LV-09 的测试和 Rust/Evidence 裁判。
