...
6. `Database.exec/query` 对内存 adapter 仍是空实现。它们不是可供业务调用的可替换 interface；在 adapter seam 被正式重塑前，不得向外扩散。
...
2. `Next candidate`：导入状态、检查点和原子合并。先设计内部 adapter interface，再由 AC-LV-03 至 AC-LV-09 的测试和 Rust/Evidence 裁判。
