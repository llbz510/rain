...
| Controlled GPU Artifact Build | 从后续受审查 tooling checkout 驱动标准 hosted Windows；单独 checkout 精确、clean candidate source，以同一固定 CMake 4.0.0 构建 GPU worker 和独立 Tauri/Cargo 主产物；native CUDA/LLVM/NSIS 与 CMake 交易由深 `controlled-toolchain-install` module 负责，只有完成标记后才可消费 CMake root；以 source-derived 文件名/kind 和基本 MZ/PE 形状识别 installer，在唯一 runner-TEMP 中真实静默安装，从实际 installed tree 反算布局/manifest，同时保留原 installer archive 解包 hygiene Judge 作为增量扫描：仅当 7-Zip tree 非空、含 AMD64 Rain 与 hash-matched release payload 时才可声明 archive scope；读取完成后静默运行生成的 uninstaller 并验证完整 payload/系统副作用清理，finally 聚合删除安装根、archive 根、owned target 与 CMake TEMP；先上传 core，再把 upload digest 固化到独立 build record 和管理员 launcher | 把 tooling commit 当 candidate target、把任意同名 PE 当 installer、以解包树替代真实 installed tree 作为布局或 manifest 来源、把空/未验证的 archive scope 声称为 clean、构建本机 candidate、把 record/launcher 放入 first upload、发布 GitHub Release、运行 GPU/model/LLM Evidence、签名或法律批准 | `scripts/release-artifact-generator.psm1`、`scripts/build-whisper-cuda-worker.ps1`、`scripts/controlled-build-disk.psm1`、`scripts/controlled-toolchain-install.psm1`、`scripts/generate-nvidia-evidence-admin-launcher.ps1`、`.github/workflows/controlled-gpu-artifact-build.yml`；candidate provenance 仍由 NVIDIA Evidence runner 验证 |
