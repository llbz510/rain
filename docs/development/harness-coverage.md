...
`Historical Evidence` 是 Evidence 的来源/时效限定词，不是新的覆盖等级：它表示证据包仍能证明其记录的历史目标或不可变产物事实，但不能签发当前目标。当前 `Strong` 行为覆盖和当前目标的 Evidence 缺口必须分开陈述。
...
| AC-LV-03 | `pipeline-asr.test.ts`、真实证据 | Strong + Historical Evidence | 已覆盖无模型、坏路径、无 demo fallback、ASR 失败不进 Stage2；schema v2 全量运行只证明 408b6db-era 路径，精确 RC 必须重跑 |
| AC-LV-04 | `database-recovery.test.ts`、`pipeline-asr.test.ts` | Strong | 覆盖回滚和过期写入防护；Rust 与 SQLite 的真实事务由 E2E 补充 |
| AC-LV-05 | `stage2-runner.test.ts`、M04/M18 Harness、证据校验器 | Strong + Historical Evidence | M04/M18 已迁移到当前 `stage2-contract` / `stage2-runner`，覆盖缺失、重复、外来、树错误、分块、重试、确定性合并和精确覆盖；结构化产物是 408b6db-era 历史事实，精确 RC 必须重跑 |
| AC-LV-06 | `pipeline-asr.test.ts`、M03 Harness | Strong | M03 通过真实数据库状态转换和控制器检查批准路径，不再直接给对象赋值 |
| AC-LV-07 | `asr-abort.test.ts`、Pipeline 测试、取消证据 | Strong + Historical Evidence | 真实证据包含取消事件链；该取消事件链属于 408b6db-era 历史 Evidence；当前 ASR lifecycle/worker 路径和精确 RC 仍需真实重放 |
| AC-LV-08 | `pipeline-recovery.test.ts`、Stage2 检查点测试、重试证据 | Strong + Historical Evidence | 当前测试覆盖复用 ASR 和重跑坏检查点；restart-proof 属于 408b6db-era 历史 Evidence，精确 RC 必须重跑 |
| AC-LV-09 | `validate-evidence.ps1`、数据库摘要、WebDriver DOM 状态、学习页截图 | Historical Evidence / current target Gap | schema v2 同时要求真实数据库内容与生产学习页、播放器、段落可见；普通单元测试或仅有 PNG 不能替代；Study/SQLite 路径在 408b6db 后实质变化，现包不能签发当前目标，精确 RC 必须重跑 |
| AC-LV-10 | `video-list-page-recovery.test.tsx`、`video-list-import.test.tsx` | Strong | 覆盖事件驱动 UI 和持久化终态；真实事件链由 E2E 补充 |
| AC-LV-11 | `validate-evidence.test.ts`、`validate-evidence.ps1` | Strong + Historical Evidence | 覆盖哈希、乱码、demo、CUDA、结构、取消、重启、生产学习页 DOM、截图和秘密；schema v2 不再把“任意 PNG”当作 UI 已就绪。外部 PowerShell 集成 Judge 使用局部 30s 环境预算，所有正负断言和验证器脚本保持不变；现包字节未变且仍通过，但 manifest 无 commit 字段，只能作为 408b6db repository-associated 历史 Evidence，不能签发当前 RC |
| AC-LV-12 | 上述能力/运行时测试、`qwen-health.test.ts`、`live-qwen.test.ts`、`validate-evidence.test.ts`、`validate-evidence.ps1`、真实 E2E Runner、`rain-real-e2e-20260726-195652` | Strong + Historical Evidence（限定 408b6db-era 组合；current target Gap） | 三角色探针、角色门禁、导入门禁和文本助手门禁均复用生产接口。schema v2 已真实验证 `ggml-large-v3.bin` CUDA ASR + DashScope `qwen3-omni-flash` 结构化与文本助手：当前默认配置、设置连接测试、可选 live smoke 和 E2E 默认值已对齐，但 live smoke 没有 Key 时必须跳过，不能代替 Evidence。该结论只覆盖此配置指纹和文本助手，不推广到其他 OpenAI-compatible 模型或 vision；其他配置仍需各自探针和完整 E2E。旧 schema v1 证据继续按原固定运行时规则验证；当前 Strong 门禁仍有效，但 isolated CUDA worker/Auto 与精确 RC 的完整组合必须重跑 |
...
| AC | 当前裁判 | 等级 | 当前结论与缺口 |
| --- | --- | --- | --- |
| AC-ST-01 | `database-content.test.ts`、`store-zustand-phase2.test.tsx`、`study-load.test.tsx`、schema v2 DOM/数据库证据 | Strong + Historical Evidence | SQLite adapter 的 Node/Sentence 字段、查询范围和行映射有直接裁判；成功路径从真实数据库一次加载同一视频的元数据、结构、句子和笔记；缺失视频、非 ready 状态和缺少段落/句子的假 ready 记录都会留在列表并显示错误，不再静默进入空学习页；schema v2 DOM/数据库包只证明 408b6db-era 学习页，当前 `StudyInterface` 的精确 RC 桌面 Evidence 仍缺失 |
...
| AC-ST-07 | `assistant-context.test.ts`、`study-playback.test.tsx`、能力测试、schema v2 证据 | Strong + Historical Evidence（文本） | 文本上下文、门禁、停止、迟到 token、可信引用和真实文本探针已覆盖；vision 明确不在此 AC；真实文本助手产物属于 408b6db-era 历史 Evidence，精确 RC 必须重跑 |
...
| AC | 当前裁判 | 等级 | 当前结论与缺口 |
| --- | --- | --- | --- |
| AC-HE-01 | `control-plane-validator.test.ts`、`control-plane-validator.mjs`、`package.json` | Strong（纯规则 fixture + 真实仓库命令） | fixture 锁定缺覆盖、缺 Owner/Judge、裁判文件不存在、验收状态冲突、明确当前事实降级，以及 CommonMark 0–3 空格下固定六段快照外的 H2、任意 H3–H6、Maintenance 日期式 timeline record（不误伤带日期控制文档路径）和日期 session-handoff-what-changed 时间线；`harness:control` 检查真实文档，`harness:check` 继续执行前端、E2E/普通互补构建和 Rust，不替代昂贵 Evidence 决策 |
...
| AC-HE-05 | `.github/workflows/runtime-settings-desktop-e2e.yml`、`src-tauri/src/e2e_config.rs`、`src-tauri/src/lib.rs`、`package.json`、`run-runtime-settings-e2e.ps1`、GitHub workflow `Runtime Settings Desktop E2E` | Strong（真实 target-commit Hosted Windows run） | workflow_dispatch run `30756311932` 在 merge commit `a329059b8172dab82c7326deb0af322045a0c396` 的干净 checkout 中核对固定桌面工具链和匹配 WebView2 driver，执行未带 `-SkipBuild` 的唯一公开命令，并完成真实 schema、添加、pending restart idle + 显式继续、第一次重启保留、删除、pending 结果跨重启保留和第二次重启消失；failure artifact 为 0，未使用 Rain secrets、模型调用、Whisper 下载、视频导入或 Evidence。该结论只签发此目标提交，workflow 仍为人工且非必需门禁 |
...
| ASR Transcript seam | Rust `asr_transcript` tests、`pipeline-asr.test.ts`、真实证据 | Strong + Historical Evidence | `AC-LV-03` 的 fail-closed 结果门禁由真实 `build_asr_transcript` 直接裁判：覆盖空结果、空白/乱码、无效/逆序/重叠时间戳、无词级时间戳回退、可疑 token、长段分句和全局唯一 ID；Pipeline 测试继续证明失败不进入 Stage2。408b6db-era Evidence 补充历史 Whisper 运行结果，但当前 isolated-worker/精确 RC 必须重跑；ASR 执行生命周期仍由 scheduler/events/command 测试和历史 Evidence 分别托管 |
| ASR Execution seam | Rust `asr_execution`/`scheduler` tests、`commands_harness`、`whisper_harness`、`pipeline-asr.test.ts`、取消证据 | Strong + Historical Evidence | `AC-LV-03/07/10` 的一次执行由 `execute_asr` 托管；私有 backend/reporter seam 直接裁判 10/35/90/100 进度顺序、成功终态、后端失败和转换中取消，scheduler Harness 锁定串行化与过期任务隔离，Whisper Harness 锁定真实取消下沉。Pipeline 与 408b6db-era Evidence 补充跨前后端状态；当前 worker/lifecycle 的精确 RC Evidence 仍缺失 |
...
| AC-RL-08 | `whisper-backend-preference.test.ts`、Rust `whisper_backend` tests、本机 GPU smoke、`nvidia-release-evidence.test.ts` | Partial | Auto/Forced/错误分类及单机 GPU 短样本存在；M3-S3 行为合同通过 PowerShell external/deep-module interface 裁判：独立 expected manifest hash、完整 manifest/installer/payload/PE/全树合同、原子脱敏 Evidence 保持；180 秒、5,760,044-byte 确定性 PCM fixture 在 `%TEMP%` 流式生成，创建/释放/Action/删除失败均可观察且聚合 fail-closed。取消窗口使用 progress callback 捕获的绝对 epoch 时间和序号到 `cancel_import` 命令完成时间，延迟超过 2 秒会拒绝；session process-start adapter 使用真实 `-Action` PSEventJob + ConcurrentQueue，以精确 Rain PID/path/start time + WebDriver ancestry 捕获短命 worker，并在 read/complete 前核对唯一 subscriber、provider/queue token、running job、root identity，隔离其他实例，拒绝 PID 复用及 subscription missing/duplicate/stopped/failed。runner 不再依赖全系统 `Get-Process -Name` 或静默空事件集；readiness 会真实 register/health/read/unregister，当前本机账户被 `Win32_ProcessStartTrace` 拒绝时按合同 fail-closed。该结果只是 adapter enablement，不是 GPU/installer Evidence；仍须在具备权限、审查并保护合并后的目标安装器上真实执行 Auto/Forced/CPU/取消/worker 失败/模型错误 Judge，`AC-RL-08` 保持 Partial |
...
| AC-RL-12 | `release-artifact-contract.md`、普通/E2E 构建隔离扫描、controlled artifact generator/workflow 双树 scanner | Partial | 已拒绝 E2E 标记进入普通前端产物；controlled workflow 现在保留原解包 Judge，以 7-Zip 解包 installer archive，并与真实 silent-install 后的 installed tree 分别执行 secret/path/DLL/用户数据/调试产物扫描。fake-tree 与静态 workflow 合同已覆盖，但在精确 target 上的 hosted run 尚未执行，因此不得升级为 Strong 或 Release Evidence |
...
| AC-VL-01 | `video-import-task-dialog.test.tsx`、`video-list-page-recovery.test.tsx`、M17 | Partial | 生产 `VideoListPage` + 公共内存数据库 + 既有 progress listener 的局部 DOM Judge 已合并入既有 recovery 文件，锁定持久 `pending` 显示“排队中”、持久 `failed` 显示“失败”且保留错误、无 live progress 的持久 `processing` 显示“正在处理”并保留阶段细节，以及实时 callback 后 `processing` 显示“正在处理 47%”并保留阶段细节。它只补卡片粗状态文字，规定信息层级的其余部分与完整 Visual Evidence 未裁判，AC 仍为 Partial |
...
| AC-SU-01 | M05、`study-navigation.test.tsx`、`study-playback.test.tsx` | Partial | 生产 `StudyInterface → CatalogBar` Judge 已证明章节/小节顶行、段落底行均横向不换行，公开点击仍经既有 `onSeek` 写入真实 Store `playPosition`；真实 media Judge 证明当前结构/段落项各以 `scrollIntoView({ block: 'nearest', inline: 'center' })` 定位，同边界的 child section 确定性优先于父 chapter，暂停时真实 media `timeupdate` 到 10 后公开目录点击使 `playPosition` 从 10 seek 到 0 仍不强制滚动、恢复播放立即重新居中；浏览器没有该 API 时不抛。长目录的受控 DOM 几何 Judge 还证明两个真 scroll owner 仅在相应方向仍有内容时渲染 `aria-hidden`、非交互、以 `--color-bg` 为不透明端的左右渐变：中段两边同时显示，0.5px epsilon 归一化去除近端残留，并在初始布局、目录节点更新、用户 scroll、窗口 resize 及端点/无溢出状态后更新；播放位置变化不重测几何或重绑 resize。该轻量 Judge 不等同真实长目录桌面可见性或视觉 Evidence；两者及 Desktop Evidence 仍未闭合 |
| AC-SU-02 | M05、`study-playback.test.tsx`、`study-navigation.test.tsx` | Partial | 生产 `StudyInterface → CatalogBar` Judge 以真实 media `timeupdate` 和公开目录点击证明当前 chapter/section 仍只由 Store `playPosition` 推导。动效方向来自相邻只读 `playPosition` 样本的瞬时差（不驱动 current/progress，也不写入 Store/持久化），故合法嵌套 `parent[0,20]/child[5,10]` 的播放 `6→11` 为 forward、用户 seek `11→6` 为 backward；同一 current 节点的后续样本只刷新该瞬时参考而不取消刚启动的动效。连续同向切换在保持同一 scroll owner/手动 `scrollLeft` 的同时交替重启仅两套受控 A/B keyframe。Judge 直接读取生产 `src/index.css`，锁定每个 A/B keyframe 仅出现一次、from/to 各恰有一个精确的 `translateX(var(--catalog-structure-slide-offset))`/`translateX(0)`，以及唯一 `--anim-base: 200ms` token；同级 paragraph 切换保持无位移。运行中的 `prefers-reduced-motion` change 会立即取消动画/位移，偏好取消后下一次结构切换恢复动画，并在卸载时移除同一 listener。该轻量 DOM/CSS Judge 不等同真实 Tauri/Desktop/Visual Evidence，Strong、Desktop/Visual Evidence 仍未闭合 |
| AC-SU-03 | `study-layout.test.tsx`、`study-notes.test.tsx`、M08/M10 | Partial | 生产 `StudyInterface` Judge 覆盖两侧 retained mount：已有 Note 的 native `hidden`/a11y/Tab 隔离与实际 memory 写入计数；以及 `loadVideo` 后以真实 `aiPanelState: 'notes'` fixture 首次挂载 AI hidden、具名 ChatInput 默认 a11y 不可见且 Tab 不进入、公开 Tab 往返后未发送草稿恢复。可见 AI wrapper 仍为列 flex；mocked `streamAiChat` 在初始隐藏、Tab 往返及输入未发送时均为 0。新增同一生产页面/Store/公开 callback seam Judge：流中切 AI→随记 不 cleanup 或重复请求，hidden 期间 token 与完成仍归原请求，切回显示连续回答；每个 request 有独立 cleanup，第一条完成后始终 0，第二条流切回后“停止”仅 cleanup 第二条一次，第一条 stale token/done 后即时断言第一回答不变、第二回答仍空且流仍 active，第二条晚到 token 不改当前回答。未完成其余隐藏面板副作用与 Desktop Judge 仍未闭合 |
