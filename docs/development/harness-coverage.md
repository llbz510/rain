...
| AC-SU-01 | M05、`study-navigation.test.tsx`、`study-playback.test.tsx` | Partial | 生产 `StudyInterface → CatalogBar` Judge 已证明章节/小节顶行、段落底行均横向不换行，公开点击仍经既有 `onSeek` 写入真实 Store `playPosition`；真实 media Judge 证明当前结构/段落项各以 `scrollIntoView({ block: 'nearest', inline: 'center' })` 定位，同边界的 child section 确定性优先于父 chapter，暂停时真实 media `timeupdate` 到 10 后公开目录点击使 `playPosition` 从 10 seek 到 0 仍不强制滚动、恢复播放立即重新居中；浏览器没有该 API 时不抛。长目录真实滚动可见性、边缘渐隐和 Desktop Evidence 未闭合 |
