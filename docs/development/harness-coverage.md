...
| AC-HE-05 | `.github/workflows/runtime-settings-desktop-e2e.yml`、`src-tauri/src/e2e_config.rs`、`src-tauri/src/lib.rs`、`package.json`、`run-runtime-settings-e2e.ps1`、GitHub workflow `Runtime Settings Desktop E2E` | Gap（真实 hosted Judge 仍为 RED） | 仅手动、只读、无 secrets 的干净 Windows Owner 已在默认分支运行；run `30339238218` 证明构建与精确 WebView2/driver 配对成功，但暴露进程环境参数未进入 Tauri window context 的 `DevToolsActivePort` RED。当前最小实现只在 Runtime Settings E2E 模式注入该参数；必须由修复后目标提交的新 workflow_dispatch GREEN 才能提升为 Strong |
