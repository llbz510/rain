...
GitHub Actions workflow `Runtime Settings Desktop E2E` 提供 `AC-HE-05` 的独立环境 Owner。它只允许 workflow_dispatch，在目标提交的干净 `windows-2025` checkout 中安装固定 Node/Rust/LLVM/`tauri-driver`、机械要求 CMake 4+，使用 runner 成对提供且版本完全匹配的 Edge 与 `msedgedriver`，然后执行未带 `-SkipBuild` 的同一公开命令。workflow 不持有写入凭据，不接收 Rain secrets，也不复制本页前述产品断言。
