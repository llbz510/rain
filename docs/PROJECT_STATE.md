...
21. `AC-LV-19` makes non-ready card navigation side-effect free and allows explicit cancellation to close a restart-stale `processing` record, but Rain still has no startup reconciler or explicit start action for a persisted `pending` record whose in-memory Owner disappeared when the app process stopped. This must receive a separate product decision, startup/restart Owner and real restart Judge before crash recovery can be called product-complete.
