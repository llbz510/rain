...
PR #48 has since merged through the protected path as `master@54c6f2422f43d8b1cacff6b314ffab8cc4ebc9a5`; its automatic Clean Windows Harness run `32928014936` is GREEN. This new, uncommitted Slice is on `codex/launch-su03-note-draft`, created from that exact `origin/master` baseline. It changes only `AC-SU-03`'s Note-draft ownership/remount gap: `StudyInterface` keeps the existing `NotesPanel` mounted behind the platform-native `hidden` attribute while AI is active. That preserves the existing `NoteItem` local textarea draft without writing it to SQLite, localStorage, or a second business state; the hidden panel is removed from layout, accessibility and keyboard navigation. The existing explicit save path and Note persistence semantics are unchanged.
