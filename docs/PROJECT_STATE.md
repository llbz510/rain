...
M1-S2, M2-S1, M2-S2 and M3-S1 are complete. PR #25's prerequisite Clean Windows Harness passed on exact head `b64a22f20962980089b08c1873b4bc8294850dbe`, the protected merge produced `master` commit `a329059b8172dab82c7326deb0af322045a0c396`, and that exact commit passed the manual Hosted Runtime Settings replay. The M2-S2 audit classifies the schema v2 package as historical target Evidence rather than current RC Evidence. M3-S1 defines the release artifact contract in `docs/development/release-artifact-contract.md`. M3-S2 has started with a no-NVIDIA CPU release-evidence runner and is now repairing the GPU release overlay so `bundle:gpu` emits the required single NSIS candidate instead of only a bare release executable. This does not yet sign `AC-RL-07` Release Evidence until a clean no-NVIDIA/CUDA Windows host runs the runner against that candidate; signing, licensing and real-video Evidence also remain absent.
