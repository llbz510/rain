...
10. `docs/development/release-scope-contract.md` — Active, user-confirmed M1-S1 Core Release scope; it fixes Launch/Post-release destinations and the single GPU-enhanced universal installer boundary, while Launch behaviors still require Confirmed ACs.
11. `docs/development/release-acceptance-contract.md` — Proposed M1-S2 Release AC matrix; its 50 candidate ACs, Evidence tiers and product details are not implementation authority until explicit user confirmation and formal AC migration.
