...
`AC-LV-21` is Confirmed and implemented on `codex/whisper-gpu-auto-fallback` from `master@c7436c4`. Runtime Settings defaults to `Auto`; the CPU-safe main process selects an isolated CUDA worker when its versioned probe succeeds and otherwise exposes the reason before using CPU. Forced CUDA fails closed, forced CPU never probes the worker, cancellation/model errors do not silently rerun another backend, and import details expose the actual backend/fallback. Local RTX 5060 Ti + large-v3 short-sample smoke and CPU-main DLL isolation passed. This is Strong behavior with a local NVIDIA smoke, not `Strong + Evidence`: a clean no-NVIDIA Windows install/CPU sample, formal GPU installer/signing, and CUDA redistribution review remain required.
...
20. Local-thumbnail creation, persistence and card rendering are Confirmed and Strong, but no Active acceptance boundary owns deletion of the app-owned thumbnail or orphan garbage collection. `DEC-PRD-060` is therefore Proposed. The already confirmed database-deletion boundary still Strongly covers atomic database cascade and preservation of the user's source video.
...
- Source branch/base: `codex/whisper-gpu-auto-fallback` from clean `master@c7436c4` (PR #22 merge). Volatile HEAD/worktree facts are not trusted here; run Git commands.
...
- Final verification passed: `npm run harness:check` completed control-plane validation, 85 frontend files / 506 tests with one explicit live-key test skipped, complementary E2E and ordinary production builds, and Rust tests (98 library tests plus existing integration suites; one real-model Harness case remains intentionally ignored). `npx tsc --noEmit`, CUDA worker release rebuild/probe/large-v3 short transcription, CPU main build/import inspection, Tauri GPU overlay debug/no-bundle build and `git diff --check` also passed. The final `dist` is the ordinary CPU-safe production artifact; the staged GPU payload remains ignored build output.
- Remaining release gates: no clean Windows machine without NVIDIA/CUDA has yet installed this target and completed app start + CPU short sample; no formal MSI/NSIS GPU install/upgrade/uninstall replay; no code-signing proof; NVIDIA CUDA runtime redistribution terms have not received a release-owner review; download/install UX for the approximately 804 MB GPU payload is not product-approved. Therefore coverage is Strong behavior + local NVIDIA smoke, with dual-environment Release Evidence still Gap.
