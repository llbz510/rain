...
Clean Windows Harness run `31808042965` for PR #33 then passed every product and release-artifact assertion (96 passed test files / 671 passed tests, with one contractually skipped live-key test), but emitted the unhandled `[vitest-worker]: Timeout calling onTaskUpdate` diagnostic. The generator file runs its real PowerShell contracts through `execFileSync`; those synchronous calls hold the Vitest worker event loop even though the individual contract assertions and their local 15/30-second budgets pass. This is a test-harness responsiveness defect, not a generator or product behavior defect.

The follow-up is test-only. A red-capable regression schedules a zero-millisecond timer while the real Windows 8.3 alias generator contract runs: before the migration it failed because the timer could not advance before the synchronous PowerShell call returned; after it, the same contract is GREEN and also confirms that no tracked PowerShell child remains. The generator, controlled-record, short-path and direct PowerShell callers now await one serial `execFile` adapter. Each invocation has a 10-second child-process budget; active children are tracked, test cleanup terminates and waits for them for up to 2 seconds, aggregates termination/settlement/TEMP-cleanup errors explicitly, and preserves owned TEMP roots if any child can still access them. Existing positive and negative cases are unchanged, and rejections now use asynchronous `rejects` assertions. The global Vitest timeout, test concurrency, retry/skip behavior and all product files remain unchanged.

Current local verification is GREEN: the event-loop regression passes, `scripts/release-artifact-generator.test.ts` passes 55/55 in 34.16 seconds, and `npm.cmd test` passes 96 files / 671 tests with one contractually skipped live-key test (97 files / 672 tests total). `npx.cmd tsc --noEmit`, `npm.cmd run harness:control`, `npm.cmd run build:e2e`, `npm.cmd run build`, and PowerShell AST parsing for all 13 script/module files pass; the two frontend builds retain only the existing Vite dynamic/static-import warnings. Branch is `codex/m3-controlled-merged-artifact-build`, current HEAD is `aa8feb20f020be9993d0cbcc039f2571c7c39083`, merge-base remains `3006757838b972b511917663e4ba8328804607d6`, and `src-tauri/target` remains absent. No Cargo/Rust/Whisper/CUDA build, installer, GPU/model/LLM execution, workflow dispatch, commit or push occurred. The uncommitted test/doc correction needs review, then a new clean-host Harness run before any hosted controlled build, artifact, Release, Evidence or AC claim.

Standards review found a P2 in the new asynchronous generator test adapter: if `afterEach` first tried to terminate a tracked child and `kill()` threw or returned false, the cleanup error was aggregated but the invocation's 10-second timer remained armed. Its later callback revisited the stored termination error outside cleanup aggregation, so it could become an unhandled asynchronous throw; the invoking Promise also remained pending until a callback that a denied child might never deliver. This was test-only lifecycle plumbing, not generator or product behavior.

The corrective TDD seam injects a fake child-process starter and captured timeout adapter. Its cleanup-first denied-kill regression was RED at 1 failed / 55 skipped: the observed invocation result was still `pending` instead of an explicit rejection. The GREEN adapter clears/takes over its timeout at cleanup entry, settles the invocation result exactly once with the cleanup failure, and allows cleanup to aggregate the same kill denial, bounded exit-settlement timeout, residual-child fact and owned-TEMP preservation. The timeout callback is safe even if reached after takeover, and a later child callback cannot overwrite the first result. The regression proves all of those facts with a fake child that refuses to exit: after cleanup and result capture, it invokes the late child callback before the final assertions verify that the first result was preserved.

Post-P2 local verification is GREEN: the focused denied-kill regression passes 1/56 in 34 ms; `scripts/release-artifact-generator.test.ts` passes 56/56 in 34.11 seconds; and `npm.cmd test` passes 96 files / 672 tests with one contractually skipped live-key test (97 files / 673 tests total). `npx.cmd tsc --noEmit`, `npm.cmd run harness:control`, `npm.cmd run build:e2e`, `npm.cmd run build`, and PowerShell AST parsing for all 13 script/module files pass. The two frontend builds retain only the existing Vite dynamic/static-import warnings. Branch, HEAD, merge-base and scope remain as above; `src-tauri/target` remains absent. No Cargo/Rust/Whisper/CUDA build, installer, GPU/model/LLM execution, workflow dispatch, commit or push occurred. This uncommitted test/doc correction requires review and another clean-host Harness run before any hosted controlled build, artifact, Release, Evidence or AC claim.

PR #33 Clean Windows Harness run `31811489847` subsequently completed successfully on exact head `dcbda56cee5ca890067235db1167610a78d69fba`. The clean hosted run passed control-plane validation, 96 Vitest files / 672 tests with one contractually skipped live-key file/test (97 files / 673 tests total), both frontend builds, and the ordinary Rust Harness after a clean 9 minute 16 second compile. Rust results were 98 library tests plus the adjacent 5, 5, 3, 5 and 5-test suites with zero failures; the existing real-model case remained ignored, and main/doc-test suites also completed with zero failures. This run proves the PR head's clean-Windows Harness, including the asynchronous PowerShell test lifecycle, but it did not execute the new manual controlled GPU artifact workflow, build an installer candidate, run a GPU/model, create Release Evidence, or advance any release AC. The PR still requires final status review and protected merge before the manual controlled build may be dispatched.

Controlled GPU Artifact Build run `31814410770`, dispatched from `master` at exact tooling commit `6427e5ff4218a4aea478d7bb092103210a652b17`, stopped in the `Download and verify pinned CUDA, LLVM, NSIS, and CMake packages` workflow step before any download. PowerShell rejected the two diagnostic strings `SHA-256 mismatch for $Uri: ...`: an interpolated variable immediately followed by a colon must use braces. This is a workflow-text ParserError, not a hash-verification, toolchain, product, artifact or release-evidence failure.

The minimal TDD correction changes only those two interpolations in `.github/workflows/controlled-gpu-artifact-build.yml` to `${Uri}:`. The new `scripts/controlled-gpu-artifact-build.workflow.test.ts` public-seam contract extracts the actual YAML `run: |` block, asks PowerShell's `Parser::ParseInput` to parse that source through an encoded parser-only command, and executes neither a workflow action nor a download. Before the YAML correction the focused test was RED (1 failed / 8 skipped, status 1) with the same variable-colon ParserError; afterward it is GREEN (1 passed / 8 skipped), and the whole workflow contract suite passes 9/9. The same contract scans every `$word:` reference in the workflow and permits only valid PowerShell scope names, confirming there are no remaining unbraced variable-colon references.

Current local verification is GREEN: `npm.cmd test` passes 96 files / 673 tests with one contractually skipped live-key file/test (97 files / 674 tests total); `npx.cmd tsc --noEmit`, `npm.cmd run harness:control`, `npm.cmd run build:e2e`, `npm.cmd run build`, and PowerShell AST parsing of all 13 script/module files pass. The two frontend builds retain only the existing Vite dynamic/static-import warnings. Branch and HEAD remain `codex/m3-controlled-download-parser-fix` / `6427e5ff4218a4aea478d7bb092103210a652b17`; `src-tauri/target` is absent. No Cargo/Rust/Whisper/CUDA build, download, installer, GPU/model/LLM execution, workflow dispatch, commit or push occurred. These three uncommitted workflow-test/state changes need review and then a fresh clean-host Harness run before any hosted controlled build, artifact, Release, Evidence or AC claim.

Controlled GPU Artifact Build run `31816497430` passed the preceding PowerShell parser boundary, then failed in the NSIS download step: the old SourceForge HTML `/download` URL produced a response whose SHA-256 began `1A96...`, not the pinned installer hash. A separate small audit of the version-specific direct URL `https://downloads.sourceforge.net/project/nsis/NSIS%203/3.11/nsis-3.11-setup.exe` observed exactly 1,564,991 bytes, an `MZ` executable header and the already-pinned SHA-256 `38D49F8FE09B1C332B01D0940E57B7258F4447733643273A01C59959AD9D3B0A`; its temporary file was removed. This establishes a URL-response failure, not a reason to weaken or change the NSIS hash.

The narrow TDD correction changes only the NSIS URL in the workflow, the release-artifact-generator pinned-toolchain contract and the NVIDIA release-evidence pinned-toolchain contract. It keeps the SHA-256 unchanged and uses the exact version-specific direct URL. Public behavior fixtures now require that direct URL; the workflow contract forbids the old `/download` string, while both PowerShell consumers explicitly reject a legacy HTML `/download` URL even when its prior NSIS SHA-256 is retained. RED was captured without a download: the workflow direct-URL contract failed, and each consumer rejected the new direct fixture while accepting the old URL. After the three URL substitutions, the focused workflow/generator/NVIDIA checks are GREEN and the complete related suites pass 9/9, 57/57 and 62/62 (128/128 total).

Current local verification is GREEN: `npm.cmd test` passes 96 files / 675 tests with one contractually skipped live-key file/test (97 files / 676 tests total); `npx.cmd tsc --noEmit`, `npm.cmd run harness:control`, `npm.cmd run build:e2e`, `npm.cmd run build`, and PowerShell AST parsing of all 13 script/module files pass. The two frontend builds retain only the existing Vite dynamic/static-import warnings. Branch and HEAD are `codex/m3-controlled-nsis-direct-download` / `e9cb1f68020b0363d2e107b10a96f8c354482844`; `src-tauri/target` remains absent. No local download, Cargo/Rust/Whisper/CUDA build, installer, GPU/model/LLM execution, workflow dispatch, commit or push occurred. This uncommitted correction needs review and a fresh controlled hosted run before any artifact, Release, Evidence or AC claim.

Controlled GPU Artifact Build run `31818875677` on exact head `82792ca8c3fa2905e7180070b41ae29962b8c4a6` passed the earlier parser and direct-URL boundaries, but `Invoke-WebRequest` still saved a dynamic SourceForge HTML response for the version-specific direct URL. Its observed hash began `97DB...`, not the unchanged pinned NSIS installer hash beginning `38D49...`; subsequent workflow work was skipped and invocation-owned cleanup succeeded. This is a Windows PowerShell transport failure, not a reason to change the direct URL or its independently verified hash.

The narrow TDD correction changes the workflow transport only. The public YAML seam first went RED without network access because the real preflight did not require/probe `curl.exe`, the download block still contained `Invoke-WebRequest`, and it lacked the fixed curl redirect/failure/output contract and immediate exit handling. The GREEN workflow now requires hosted `curl.exe`, verifies only `curl.exe --version` through the existing native probe without adding a toolchain-record field, and downloads with `curl.exe --location --fail --silent --show-error --output $Destination --url $Uri`. It captures `$LASTEXITCODE` on the next line and throws before the existing SHA-256 gate if curl fails; URL, SHA-256, reservation/cleanup behavior and record schema are unchanged. The static contract prohibits `Invoke-WebRequest`, requires those exact transport protections and keeps the hash gate after curl.

Current local verification is GREEN: the focused curl public-seam regression passes 1/10, the complete workflow contract passes 10/10, and `npm.cmd test` passes 96 files / 676 tests with one contractually skipped live-key file/test (97 files / 677 tests total). `npx.cmd tsc --noEmit`, `npm.cmd run harness:control`, `npm.cmd run build:e2e`, `npm.cmd run build`, and PowerShell AST parsing of all 13 script/module files pass. The two frontend builds retain only the existing Vite dynamic/static-import warnings. Branch and HEAD are `codex/m3-controlled-curl-download` / `82792ca8c3fa2905e7180070b41ae29962b8c4a6`; `src-tauri/target` remains absent. No local network download, Cargo/Rust/Whisper/CUDA build, installer, GPU/model/LLM execution, workflow dispatch, commit or push occurred. This uncommitted workflow/test/state correction needs review and a new controlled hosted run before any artifact, Release, Evidence or AC claim.

Controlled GPU Artifact Build run 31820757660 on exact head c69b19a7b495dcc00ddf9f73f38a1f1390a149c6 passed the corrected download transport and hash gates, then failed in Install pinned native toolchain. The production transaction invoked each downloaded installer directly and read $LASTEXITCODE; under strict mode that variable was unset at the CUDA step. The CUDA process/file was consequently still locked when both the immediate package deletion and the later invocation-owned cleanup ran. This is process-lifecycle and cleanup-order failure, not a pin, download, product, artifact or Evidence failure.

The narrow TDD correction keeps the workflow's existing Invoke-RainControlledToolchainInstall wiring and changes only its tested module behavior. The production adapter now starts each installer through Start-Process -Wait -PassThru -WindowStyle Hidden, requires a returned ExitCode, and fails closed on a nonzero or missing code; the installer path does not read $LASTEXITCODE. Installer/archive cleanup runs only after that process call returns. Downloaded CUDA, LLVM, NSIS and CMake files now use one explicit bounded delete helper: at most five attempts, with 250 ms delays between attempts, preserving every deletion/delay error in the final failure rather than swallowing a lock. The public fake-process test captures file, arguments and Wait/PassThru/hidden-window flags; transaction fakes prove the install-to-delete order, a one-time CUDA lock recovery, and all five persistent-lock failures. The workflow contract continues to require that native installation stays in the tested module and now follows the bounded CMake archive deletion seam.

Current local verification is GREEN: the focused transaction and workflow suites pass 17/17 tests (7/7 and 10/10); npm.cmd test passes 96 files / 679 tests with one contractually skipped live-key file/test (97 files / 680 tests total); npx.cmd tsc --noEmit, npm.cmd run harness:control, npm.cmd run build:e2e, npm.cmd run build, and PowerShell AST parsing of all 13 script/module files pass. The two frontend builds retain only the existing Vite dynamic/static-import warnings. This uncommitted work is on codex/m3-controlled-installer-wait from c69b19a7b495dcc00ddf9f73f38a1f1390a149c6; no local installer or download, Cargo/Rust/Whisper/CUDA build, GPU/model/LLM run, workflow dispatch, commit or push occurred. Working-tree and staged diff checks pass, and src-tauri/target is absent; this needs independent review and a fresh hosted controlled run before any artifact, Release, Evidence or AC claim.

Standards review then found two fail-closed edge cases in the same public-adapter seam. An object with an ExitCode property whose value was null, whitespace, fractional text or an out-of-range Int64 could be converted inconsistently; notably null could become zero. The bounded package deletion helper also stopped after a retry-delay failure but falsely described the maximum five attempts as having run.

The narrow follow-up TDD correction has separate RED-to-GREEN behavior tests. Installer ExitCode now accepts only a real integral numeric value in the Int32 range (including the real zero success value), rejects null, text, fractional and out-of-range values with one invalid-exit-code failure, and retains normal nonzero failure handling. The cleanup helper is mechanically renamed Remove-RainControlledToolchainPackageFile to cover CUDA, LLVM, NSIS and CMake package files. It records the actual completed delete attempts, stops rather than retrying without a successful delay, and reports that actual count plus the configured maximum while aggregating both the delete and delay failures. The delay-failure fake proves exactly one delete and one delay attempt, no un-delayed second delete, and both errors in the final transaction failure.

Current local verification is GREEN: the focused transaction and workflow suites pass 19/19 tests (9/9 and 10/10); npm.cmd test passes 96 files / 681 tests with one contractually skipped live-key file/test (97 files / 682 tests total); npx.cmd tsc --noEmit, npm.cmd run harness:control, npm.cmd run build:e2e, npm.cmd run build, and PowerShell AST parsing of all 13 script/module files pass. The two frontend builds retain only the existing Vite dynamic/static-import warnings. This remains uncommitted on codex/m3-controlled-installer-wait from c69b19a7b495dcc00ddf9f73f38a1f1390a149c6; no local installer or download, Cargo/Rust/Whisper/CUDA build, GPU/model/LLM run, workflow dispatch, commit or push occurred. Working-tree and staged diff validation pass, and src-tauri/target is absent; review and a new hosted controlled run remain before any artifact, Release, Evidence or AC claim.

Controlled GPU Artifact Build run `31824945908` passed the corrected download and native-tool installation phases, then failed while building the CUDA worker. The Rust `cmake` crate received `-DCMAKE_ROOT=D:\\a\\_temp\\rain-pinned-cmake-4.0.0`, although the pinned CMake 4 package is nested below that extraction directory. That overrides CMake's own internal module-root discovery and produced the missing `CMakeSystem.cmake.in` / unknown `_cmake_find_compiler_path` errors. This is a controlled build-environment propagation failure, not a CMake pin, compiler, product, installer, artifact, or Release Evidence failure.

The narrow public workflow/worker TDD seam first went RED without Cargo, download, installer, GPU, model, or workflow execution: 3 assertions failed while 17 remained GREEN. The transaction returned the extraction parent as `cmakeRoot`, wrote the reserved `CMAKE_ROOT` entry to `GITHUB_ENV`, and neither worker nor direct Tauri child boundary cleared it. The GREEN transaction now reports the actual nested `cmake-4.0.0-windows-x86_64` package root and exports only the intended `CMAKE_PATH` and standard `CMAKE` executable selection. The worker removes `Env:CMAKE_ROOT` immediately before its `cmd.exe`/Cargo child, and the workflow removes it immediately before its direct `npx.cmd ... tauri build` child. The toolchain record derives the package root from the verified executable, rejects a path that is not exactly `<root>\\bin\\cmake.exe`, and records both `executable` and `root`; it does not attempt to set CMake's reserved internal variable.

Current local verification is GREEN: the focused transaction/workflow suites pass 20/20; `npm.cmd test` passes 96 files / 682 tests with one contractually skipped live-key file/test (97 files / 683 tests total); `npx.cmd tsc --noEmit`, `npm.cmd run harness:control`, `npm.cmd run build:e2e`, and `npm.cmd run build` pass. Both frontend builds retain only the existing Vite dynamic/static-import warnings. PowerShell AST parsing passes for all 13 scripts/modules; working-tree and staged `git diff --check` are required again after this state update. This uncommitted work is on `codex/m3-controlled-cmake-root-env` from exact `89e1a9ea400db75e08050db311e71cf189fd3460`; no local Cargo/Rust/Whisper/CUDA build, download, installer, GPU/model/LLM execution, workflow dispatch, commit, or push occurred, and `src-tauri/target` is absent. Independent review and a new hosted controlled run remain required before any artifact, Release, Evidence, or AC claim.

### 2026-08-15 Controlled GPU Artifact Build Visual Studio 2022 selection correction

Controlled GPU Artifact Build run `31828331052` reached the CUDA worker after the preceding download and toolchain-install corrections, but its unconstrained `vswhere -latest` selection resolved a Visual Studio 18 instance with MSVC 14.51. CUDA 12.9 then rejected that host compiler with its Visual Studio 2017--2022-only `host_config` guard. The current Windows 2025 runner-image inventory separately records Visual Studio Enterprise 2022 version 17.14 at `C:\Program Files\Microsoft Visual Studio\2022\Enterprise`; the failure is therefore an uncontrolled compiler-selection problem, not a CUDA pin, download, installer, product, artifact, or Release Evidence result.

This narrow uncommitted repair is on `codex/m3-controlled-vs17-toolchain`, created from exact `origin/master` base `6c3e916924caa12cc03d73085d2b2c1b568b6738`. The public workflow/worker/cleanup seam was first RED without a local Cargo build, download, installer, GPU, model, or workflow dispatch: 3 assertions failed while 20 remained GREEN. It required the workflow's `vswhere` selection to use `[17.0,18.0)`, validate the returned installation version has major 17, derive the newest MSVC/dumpbin and `vcvars64.bat` beneath that selected root, and pass that exact `vcvars64.bat` path to the CUDA worker. The fallback in `build-whisper-cuda-worker.ps1` now uses the same 17.x interval and fails explicitly on a nonzero `vswhere` exit, blank output, or missing `vcvars64.bat`; it does not add `-allow-unsupported-compiler`. The pre-existing toolchain record continues to store the exact selected MSVC toolset directory name in `msvc.version`, so its current schema already represents the needed fact and was not expanded.

The cleanup regression uses a real temporary Git repository with 21 modified tracked files plus one untracked build residue. It proves the workflow still fails closed and never restores tracked files, removes the untracked residue, and reports only the first 20 relative porcelain entries rather than an opaque clean-status error or an absolute path. The targeted suite is GREEN at 23/23. Full lightweight verification is GREEN: `npm.cmd test` passes 96 files / 685 tests with one contractually skipped live-key file/test (97 files / 686 tests total); `npx.cmd tsc --noEmit`, `npm.cmd run harness:control`, `npm.cmd run build:e2e`, and `npm.cmd run build` pass; PowerShell AST parsing passes for all 13 scripts/modules and the 3 changed workflow `run` blocks. The two frontend builds retain only the existing Vite dynamic/static-import warnings. Working-tree/staged whitespace checks and final `src-tauri/target` absence must be reconfirmed after this state update. No local Cargo/Rust/Whisper/CUDA build, download, installer, GPU/model/LLM execution, workflow dispatch, commit, or push occurred. Independent review and a new controlled hosted run are still required before any artifact, Release, Evidence, or AC claim.

### 2026-08-15 Controlled GPU Artifact Build Windows 2022 runner correction

Controlled GPU Artifact Build run `31832043594` established that the preceding VS17-selection repair cannot succeed on the changing `windows-2025` image: the runner explicitly reported image `windows-2025-vs2026` version `20260810.198.2`, and the constrained VS17 `vswhere` query returned blank. GitHub's official `actions/runner-images` issue `#14017` records that `windows-2025` has migrated to VS2026 and that workloads requiring VS2022 must use `windows-2022`. This is a hosted-image compatibility fact for CUDA 12.9's VS2022 compiler boundary, not a reason to loosen the existing VS17 range, enable an unsupported compiler, change any tool pin/hash, or alter product/installer/Evidence behavior.

The narrow uncommitted repair is on `codex/m3-controlled-windows-2022`, created from exact `origin/master` base `869cc675b7acf67bcb2ce172f3cfd5a265fcec7f`. Its public workflow seam first went RED without local Cargo, download, installer, GPU, model, or workflow execution: the focused suite had 1 failure and 13 passes because the sole controlled `runs-on` label was `windows-2025`, not fixed `windows-2022`. The GREEN change changes only that label. The retained workflow contract now requires exactly one `windows-2022` label and rejects fixtures for `windows-2025`, `windows-latest`, `windows-2026`, `windows-2025-vs2026`, self-hosted pools, and `vars`/`inputs` expressions. It leaves the existing VS17 range/major/path checks, explicit `VcVarsPath`, pinned download URLs/hashes, CMake/disk gates, cleanup diagnostics, and candidate target untouched. The current M3 plan statements in `docs/development/agent-first-development-plan.md` and `docs/development/rain-project-delivery-plan.md` now make the same controlled-runner distinction; unrelated Harness and Runtime Settings workflows remain on their separately documented `windows-2025` runners.

Current local verification is GREEN: the focused workflow suite passes 14/14; `npm.cmd test` passes 96 files / 686 tests with one contractually skipped live-key file/test (97 files / 687 tests total); `npx.cmd tsc --noEmit`, `npm.cmd run harness:control`, `npm.cmd run build:e2e`, and `npm.cmd run build` pass; PowerShell AST parsing passes for all 13 scripts/modules. The two frontend builds retain only the existing Vite dynamic/static-import warnings. Working-tree/staged whitespace checks and final `src-tauri/target` absence must be reconfirmed after this state update. No local Cargo/Rust/Whisper/CUDA build, download, installer, GPU/model/LLM execution, workflow dispatch, commit, or push occurred. Independent review and a new controlled hosted run are still required before any artifact, Release, Evidence, or AC claim.

### 2026-08-23 Controlled GPU archive layout and isolated tracked-source build correction

The successful controlled hosted build run `31835071607` exposed two post-build control failures rather than a product-function failure: 7z's NSIS extraction places the application under a direct `$PLUGINSDIR` wrapper, while the archive validator treated the extraction root as the application root; and the worker Cargo invocation wrote Tauri-generated schema files into the canonical candidate checkout. The latter must not be hidden with `git checkout`, `git restore`, or a cleanup of tracked source files.

This uncommitted correction is on `codex/m3-controlled-archive-layout-cleanliness` at exact base `3a4e3d9688abb035ce7ca63a1b13f3830ac82cc7`. `scripts/release-artifact-generator.psm1` now accepts only one exactly named `$PLUGINSDIR` wrapper across the complete extracted tree, and requires that unique wrapper to be a direct extraction-root child; it still requires exactly one AMD64 `Rain.exe`, now at `$PLUGINSDIR/Rain.exe`, and requires the manifest, CUDA worker and three runtime DLLs to occur exactly once globally and at their exact `$PLUGINSDIR/resources/whisper-backends` locations. The payload directory itself must be exactly that five-file set. Existing archive-wide size/SHA-256 validation, duplicate detection, and hygiene/secret scanning remain additive judges; there is no recursive “first match wins” path.

The controlled workflow now uses the already verified canonical `candidate` checkout only as the exact-commit/source-proof root. After the hosted prerequisite step confirms `tar.exe`, it reserves, masks and opens an invocation-owned `RUNNER_TEMP` parent, creates its `candidate-source` child, and exports `git archive` tracked files for that precise commit only into that child. It removes the parent-level temporary archive and runs `npm ci`, the CUDA worker build and Tauri/NSIS package only from `candidate-source`; the final invocation-owned ownership sweep removes the reserved parent and therefore its child. Its `always()` source check now fails closed if the canonical candidate becomes dirty and never cleans, checks out, restores, or otherwise masks such a change. The source-tree reservation token is masked before writing to `GITHUB_ENV`. The same ownership-hygiene check found and repaired an existing leak: `PINNED_DOWNLOADS_RESERVATION_TOKEN` is now also masked before it is written to `GITHUB_ENV` (the real run log had shown it unmasked).

TDD evidence is local and lightweight only: the new `$PLUGINSDIR` acceptance test was first RED with `Installer archive CUDA payload manifest must be located at resources/whisper-backends/payload-manifest.json`; after the validator change its focused suite is GREEN (7 passed), including a nested second exact-name wrapper that first RED and is now rejected. The isolated-tree and token-masking workflow contracts were first RED (2 failed) and then GREEN; the follow-up pure-child contract was also first RED and then GREEN. The full workflow-contract suite is GREEN (16/16).

Final allowed local verification is GREEN: `npm.cmd test` passes 97 files / 700 tests with one contractually skipped live-key file/test (98 files / 701 tests total); `npx.cmd tsc --noEmit`, `npm.cmd run harness:control`, `npm.cmd run build:e2e`, and `npm.cmd run build` pass. The final `dist` is the ordinary production frontend artifact, and both builds retain only the existing Vite dynamic/static-import warnings. PowerShell AST parsing passes for all 14 script/module files, while the workflow contract parser-checks all 15 `run` blocks with GitHub expressions normalized only in the parser copy. Working-tree whitespace validation passes and `src-tauri/target` remains absent. No local Cargo/Rust/Whisper/CUDA build, installer/download, GPU/model/LLM run, `harness:check`, workflow dispatch, commit, push, PR, or release action occurred. A new hosted controlled run is still required; its successful artifact remains only a candidate package, not GPU evidence, a Release, or an AC upgrade.

### 2026-08-23 Standards review: controlled candidate-source deep module repair

The first independent Standards review of the preceding isolated-source workflow change was `FAIL`: downstream npm/worker/Tauri/artifact consumers did not reopen the cross-step ownership reservation, and the workflow text contract had replaced rather than complemented a real temporary-Git behavior judge. This was a control-plane defect, not a product, installer, GPU, model, Evidence, or Release result.

The corrective deep module is `scripts/controlled-candidate-source.psm1`, with only `New-RainControlledCandidateSource` and `Open-RainControlledCandidateSource` as its external interface. `New` reserves an owned parent, verifies ownership, exports the exact commit with real `git archive`, extracts with real `tar.exe` into only its `candidate-source` child, removes the temporary tar, and aggregates primary/archive/owned-cleanup failures. `Open` revalidates owned root, token, owner, authority, exact child path and existence. The workflow now masks and exports explicit `BUILD_SOURCE_OWNED_ROOT`, `BUILD_SOURCE_ROOT` and token fields; npm, worker, Tauri and artifact consumers each reopen before use, while the final existing ownership sweep removes the reserved parent.

TDD evidence: the new behavior suite first RED because its module was absent, then its three independently added failure slices first RED until the test state correctly observed their public result. It is now GREEN at 5/5, using a real temporary Git repository to prove tracked-only export, no `.git`, no node_modules/target/untracked copy, and unchanged canonical tracked plus three-category untracked bytes/status. It also proves wrong token/owner/path failures; a git failure removes both the owned root and reservation without reaching tar; a successful real `git archive` reaches the injected tar failure and also cleans both; and a primary failure plus denied owned cleanup aggregates both errors while deliberately leaving the reservation for test-only `afterEach` recovery. The workflow contract is GREEN at 16/16. The complete allowed verification is the 97-file / 700-test result, TypeScript/control/E2E/ordinary-build/14-file AST/whitespace/target-absence result recorded immediately above. No Cargo/Rust/Whisper/CUDA build, installer/download, GPU/model/LLM execution, `harness:check`, dispatch, commit, push or PR occurred. This requires renewed independent Spec and Standards review before a protected PR or hosted run.

The renewed independent reviews of the complete uncommitted diff both returned `PASS`, with P0/P1/P2 all empty. Spec confirmed the unique direct `$PLUGINSDIR` wrapper, unique AMD64 `Rain.exe`, exact manifest/worker/three-DLL payload, retained global duplicate/hash/size/secret/hygiene judges, tracked-only exact-commit build source, cross-step ownership reopening, fail-closed cleanup and no Evidence/Release/AC promotion. Standards confirmed all first-review findings closed: each consumer reopens the reservation; real temporary-Git behavior tests complement the workflow text contract; owned root and pure source child have distinct names and interfaces; git/tar/cleanup failures and aggregate errors are covered; canonical tracked and untracked bytes/status remain unchanged; both affected reservation tokens are masked; and no restore/checkout/clean masks tool changes. The only remaining gates for this Slice are the protected PR's Clean Windows Harness and a post-merge controlled hosted build; even a successful hosted artifact remains only a candidate package.

### 2026-08-23 Clean Windows Harness short-path return repair

Protected PR #41's `Clean Windows Harness` run `32643011665` supplied a real hosted RED in `scripts/controlled-candidate-source.test.ts:64`: `New-RainControlledCandidateSource` returned an `8.3` caller path under `C:\Users\RUNNER~1\...\candidate-source`, while `Open-RainControlledCandidateSource` returned the same directory after `Get-Item` expanded it to `C:\Users\runneradmin\...\candidate-source`. This was a public-interface representation defect, not an ownership-validation failure; accepting a mismatched path or weakening the equality assertion would have hidden it.

The narrow repair makes both public results use the same normalized caller-path representation: absolute with dot segments removed, but without filesystem canonicalization that can expand an `8.3` alias. `Open` still first validates the owned root's exact reservation, owner, token and authority, then verifies the exact `candidate-source` child and its existence. The existing public identity assertion remains unchanged; it is the hosted RED judge. The local focused temporary-Git suite is GREEN at 5/5 after the repair; its local temporary root does not reproduce the hosted short-path alias. The complete allowed suite remains GREEN at 97 passed files / 700 passed tests plus one contractually skipped file/test, followed by TypeScript, control-plane, E2E and ordinary production builds, PowerShell AST for all 14 scripts/modules, whitespace validation and final `src-tauri/target` absence. Both builds emitted only the existing Vite dynamic/static-import warnings and the final `dist` is ordinary production output. No Cargo/Rust/Whisper/CUDA build, `harness:check`, installer/download, GPU/model/LLM execution, dispatch, commit, push or PR update occurred. The PR still needs fresh independent Spec/Standards review and a fresh protected Clean Windows Harness result before further hosted work.

The independent Spec and Standards reviews of this two-file repair both returned `PASS`, with P0/P1/P2 empty. Both confirmed that only the public returned path representation changed; the existing strict equality Judge was not edited, and reservation/owner/token/authority validation, exact child comparison, existence checking, cleanup aggregation and cross-step ownership behavior remain intact. They also confirmed this record accurately preserves the failed hosted run and candidate-only release boundary. A conclusion-only status sync must be rechecked before the repair commit; after that, PR #41 still requires a new protected Harness run.

### 2026-08-23 Controlled hosted module-import scope repair

Controlled hosted run `32644328493` produced a real RED before any native compilation: its build step imported `controlled-owned-directory.psm1`, then imported `controlled-candidate-source.psm1` with the workflow's existing `-Force`; after that sequence `New-RainControlledDirectoryReservation` was no longer callable. The cause was the candidate module's nested forced reimport of the owned-directory module, which replaced the caller's exported command surface. This is an import-scope control defect, not a reason to weaken ownership or skip the reservation.

The new executable PowerShell behavior test first reproduced the same import order locally and RED with the same missing-command error. It then proves that the two public surfaces coexist operationally: it creates an owned reservation, creates its exact `candidate-source` child, uses candidate-source's cross-step `Open`, independently reopens the owned reservation, and removes it through the owned cleanup entry. The narrow correction removes only the nested `-Force` from the candidate module's dependency import; the workflow still imports its explicitly controlled modules with `-Force`. The focused candidate-source suite is GREEN at 6/6 and the combined candidate/workflow suites are GREEN at 22/22 after the correction. Complete allowed verification is GREEN at 97 passed files / 701 passed tests plus one contractually skipped file/test, followed by TypeScript, control-plane, E2E and ordinary production builds, PowerShell AST for all 14 scripts/modules, whitespace validation and final `src-tauri/target` absence. The builds emitted only the existing Vite dynamic/static-import warnings and final `dist` is ordinary production output. No Cargo/Rust/Whisper/CUDA build, `harness:check`, installer/download, GPU/model/LLM execution, dispatch, commit, push or PR action occurred locally. A fresh protected review and hosted controlled run remain required before any artifact, Release, Evidence or AC claim.

Independent Spec and Standards review both returned `PASS`, with P0/P1/P2 empty. Spec confirmed the real hosted failure is directly closed without changing workflow top-level forced imports or weakening reservation, owner, token, authority, exact-child, cross-step reopen or cleanup behavior. Standards confirmed the nested non-forced dependency import is the minimal stable PowerShell module-scope correction, does not depend on stale modules or accidental import order, and that the executable behavior Judge proves both public surfaces can coexist through their real operations. This conclusion-only state sync must receive a final read-only recheck before commit; the protected PR Harness and a new post-merge controlled hosted run remain mandatory.

### 2026-08-23 PR #42 hosted coexistence-test filesystem-identity repair

PR #42's hosted `Clean Windows Harness` run `32645461910` supplied a new RED at the coexistence behavior test's former JavaScript string assertion: Node retained the temporary root's short spelling while PowerShell returned the same already-existing source with its long spelling. The reservation, candidate cross-step `Open`, owned re-open and cleanup all succeeded, and the original `New`/`Open` strict public returned-path equality test also passed. Therefore this was a test representation mistake, not a module-contract failure; changing the module or weakening the original strict judge would have been incorrect.

The coexistence test now compares filesystem identity inside PowerShell while both directories still exist: it resolves the returned and expected source paths with `Get-Item`, compares their `FullName` values ordinal-ignore-case, and independently performs the same check for the owned reservation path. It then performs the unchanged controlled cleanup and asserts that the owned root is absent. The original strict `New`/`Open` equality assertion remains unchanged. The focused candidate-source suite is GREEN at 6/6 and the combined candidate/workflow suites are GREEN at 22/22 after this test-only repair. The complete allowed suite is also GREEN at 97 passed files / 701 passed tests plus one contractually skipped file/test; both unchanged NVIDIA release-evidence tests that timed out in the hosted run passed within their existing contract locally. TypeScript, control-plane, E2E and ordinary production builds, all-14-file PowerShell AST, whitespace and target-absence checks also pass; builds retain only the existing Vite warnings and final `dist` is ordinary production output. No timeout, Harness, or evidence-test behavior is changed without a separate approved Harness Migration, so a fresh protected run must still assess the hosted result. No Cargo/Rust/Whisper/CUDA build, `harness:check`, installer/download, GPU/model/LLM execution, dispatch, commit, push or PR update occurred locally.

Independent Spec and Standards review of this test-only repair both returned `PASS`, with P0/P1/P2 empty. They confirmed filesystem identity is checked only for existing test directories, while the original strict public returned-path Judge and all production ownership/path contracts remain unchanged. They also confirmed that retaining the two hosted timeout facts, making no timeout/Harness change without repeatable evidence and requiring a fresh protected run is the correct fail-closed boundary. This conclusion-only state update requires final read-only recheck before the PR is updated.

### 2026-08-23 Authorized Harness Migration: atomic NVIDIA provenance Judges

The user explicitly authorized this Harness Migration with “可以，别忘了更新计划”. The exact old aggregate provenance Judge in `scripts/nvidia-release-evidence.test.ts` was one default-five-second `it` containing four serial real PowerShell `invokeContract` calls. Protected `Clean Windows Harness` runs `32645461910` and `32646313003` both RED at that same outer Judge after `5057ms` and `5069ms`; neither reported a failed provenance assertion. The command-line-only temporary `--testTimeout=1000` RED reproduced the aggregate-timeout shape locally in `1030ms` without changing Vitest configuration.

The approved replacement is four independently isolated, default-five-second provenance Judges: accept the independent record, reject missing `coreArtifact`, reject wrong artifact-manifest SHA-256, and reject a tooling commit inconsistent with the artifact manifest. Each creates its own fixture, invokes the same PowerShell contract exactly once, and retains the old success/rejection assertion. The child `contractProcessTimeoutMs=4000`, production module/runner, global timeout configuration, other NVIDIA tests and all Evidence behavior remain unchanged. The focused four cases are GREEN at 4/4 (65 file tests total, 61 intentionally skipped by the name filter). The formal contract, authorization, retired aggregate Judge, mappings, boundaries and verification gates are in `docs/development/harness-migration-2026-08-23-nvidia-provenance-atomic-tests.md`.

`AC-RL-08` remains `Partial`; this change generates no artifact, Release, Evidence or AC upgrade. The two other NVIDIA release-evidence outer timeout observations are explicitly out of scope and receive no timeout/Harness change here. Full allowed local verification and independent Spec/Standards review have now passed as recorded below; a fresh protected Hosted Harness remains required, and no Hosted GREEN may be claimed until that run passes. No Cargo/Rust/Whisper/CUDA build, `harness:check`, installer/download, GPU/model/LLM execution, dispatch, commit, push or PR action occurred in this migration.

The completed allowed local verification is GREEN: `npm.cmd test` passed 97 files / 704 tests with one contractually skipped file/test; `npx.cmd tsc --noEmit`, `npm.cmd run harness:control`, `npm.cmd run build:e2e`, the final sequential `npm.cmd run build`, PowerShell AST parsing for all 14 scripts/modules, `git diff --check`, and the `src-tauri/target` absence check all passed. An intermediate verification attempt incorrectly ran the E2E and production builds concurrently against their shared `dist` directory; the production isolation guard correctly rejected the E2E markers. The required sequential production rerun then passed and left ordinary production output. This was verification-command interference, not a product or repository defect.

### 2026-08-23 User-directed plan reprioritization after controlled-build closeout

After the user asked why planned functionality was not being completed first, the user accepted the proposed transition and instructed “可以，别忘了更新计划”. The same reply explicitly authorized both the narrow Harness Migration and the required Active-plan synchronization. The plan synchronization is control-plane maintenance rather than a second product implementation Slice: it changes no AC, Judge, Evidence tier or product behavior, and is reviewed in the same stable diff so a later session cannot follow the superseded sequence. The two Active plans now require the already-started controlled merged-target build Slice to close first, including protected merge and one GREEN controlled hosted build. They then pause further M3 GPU Evidence, signing, licensing and distribution expansion for a read-only Launch functionality completeness audit. That audit must trace Confirmed ACs to real production routes, UI, public interfaces and current Judges, classifying each as actually usable, partial, test/shadow-only or missing; it must not infer completion from PRD text, a test file or a buildable candidate.

After the audit, work returns to one user-visible Confirmed functionality gap at a time under TDD, allowed gates, independent Spec/Standards review and protected PRs. Remaining M3 real NVIDIA Evidence and release work resumes only after the Launch feature surface is stable. This sequencing change modifies neither AC semantics nor coverage tier and does not authorize Post-release translation, Vision or advanced tree editing. It is recorded consistently in `docs/development/agent-first-development-plan.md` and `docs/development/rain-project-delivery-plan.md`.

Independent Spec and Standards review of the complete stable diff now both return `PASS`, with P0/P1/P2 empty. The first Standards pass correctly rejected an ambiguous migration write boundary, an internally conflicting delivery critical path and stale plan dates. The corrected records distinguish the narrow test migration from the separately user-directed plan synchronization, align both Active plans and the delivery critical path on controlled-build closeout → Launch functionality audit/user-visible Confirmed gaps → remaining M3 release work, and update both plan dates to 2026-08-23. Spec confirmed the four old provenance observations remain one-for-one, `AC-RL-08` remains `Partial`, and no AC, product behavior, Post-release scope, Evidence tier or Release claim was added. Standards confirmed the atomic fixture/process/cleanup/timeout behavior and all control boundaries remain intact. A fresh protected `Clean Windows Harness` remains before merge; no Hosted GREEN is claimed yet.

### 2026-08-24 Controlled archive layout correction: `$INSTDIR` is the application root

Post-merge controlled hosted run `32650125893` (tooling `50486d1`, candidate `3006757838b972b511917663e4ba8328804607d6`) completed the heavy Windows compilation and packaging work, then failed at the archive Judge with `Installer archive Rain executable must be located at $PLUGINSDIR/Rain.exe.` The failure identifies a wrong archive-layout inference in the Judge, not a successful artifact, Release, Evidence or AC result.

The historical interpretation in sections 2259, 2261, 2265 and 2277 is now explicitly superseded while those entries remain preserved as audit history. It incorrectly treated `$PLUGINSDIR` as the unpacked application root. The candidate locks `@tauri-apps/cli` at `2.11.4`; the [Tauri `tauri-cli-v2.11.4` NSIS template](https://github.com/tauri-apps/tauri/blob/tauri-cli-v2.11.4/crates/tauri-bundler/src/bundle/windows/nsis/installer.nsi) places the main binary and resources after `SetOutPath $INSTDIR`. The [7-Zip NSIS reader](https://github.com/ip7z/7zip/blob/f9d78aff31a5f2521ae7ddbdc97c4a8855808959/CPP/7zip/Archive/Nsis/NsisIn.cpp) initializes its archive prefix with `$INSTDIR` and keeps `$PLUGINSDIR` as a separate absolute variable. Therefore the controlled 7-Zip archive contract is one direct `$INSTDIR` application root plus one direct `$PLUGINSDIR` plugin directory.

The corrected Judge counts exact-name directories across the complete extracted tree, requires exactly one of each and requires both to be direct extraction-root children. It requires the globally unique AMD64 `Rain.exe`, payload manifest, CUDA worker and three CUDA DLLs at exact `$INSTDIR` paths; the application payload directory remains the exact five-file set. Size/SHA-256, global duplicate detection, secret and hygiene scans remain additive, and no recursive first-match behavior or unsupported plugin-file whitelist is introduced. New fixture coverage starts from that official dual-directory layout and rejects bare/arbitrary application roots, nested duplicate `$INSTDIR` or `$PLUGINSDIR`, payload moved into the plugin directory, external duplicate worker, and payload supersets. The initial focused official-layout fixture was RED with the hosted error and is GREEN after the correction.

The subsequent independent Spec/Standards review correctly found P1/P2 coverage gaps. TDD remediation adds public-generator negative fixtures for a missing plugin directory, a sole nested plugin directory, a sole nested application root, a second AMD64 `Rain.exe` anywhere in the archive, and case-mismatched `$instdir`/`$pluginsdir` names. To prove the direct-child fixture is a real Judge, a temporary local removal of the `$INSTDIR` direct-child check produced RED because that malicious layout was accepted; the check was immediately restored and all six new focused cases returned GREEN. The broad `Get-RainReleaseArtifactRelativePath` diagnostic-redaction edit was intentionally withdrawn: no public behavior could reach that private helper's out-of-root branch, so the repository retains no untested diagnostic change. The complete archive generator suite is GREEN: 71/71 tests passed.

`AC-RL-12` remains `Partial`. Focused archive tests, `tsc --noEmit`, control-plane validation, E2E/production frontend builds, PowerShell AST and diff/target checks passed locally. The first complete `npm test` run on this Chinese-path workspace recorded one intermittent existing path-encoding failure in `scripts/validate-evidence.test.ts` (`D:\\xiangmu\\rain 未完成` became `D:\\xiangmu\\rain δ���` across a child-process boundary): 96 files passed, 1 skipped and 1 failed. That record is retained and is not attributed to the archive correction. A subsequent isolated rerun of `scripts/validate-evidence.test.ts` passed 18/18. The following complete rerun, before the six double-review negative fixtures were added, passed 97 files with 1 skipped and 705 tests with 1 skipped. The final stable-diff complete rerun passed 97 files with 1 skipped and 711 tests with 1 skipped; `tsc --noEmit`, `harness:control`, and sequential `build:e2e` then `build` also passed on that stable diff. The second independent Spec and Standards review both returned PASS with P0/P1/P2 empty: the first-review gaps were closed by the six public negative fixtures, a mutation RED→GREEN proof, and withdrawal of the untestable diagnostic edit. Therefore the current complete frontend-test state is GREEN. A protected PR, Hosted Harness, and a fresh controlled run are still pending; `AC-RL-12` remains `Partial`. No local Cargo/Rust/Whisper/CUDA build, `harness:check`, installer/download, GPU/model/LLM execution, dispatch, commit, push, artifact, Release or Evidence action occurred for this correction.

### 2026-08-24 Controlled archive layout correction: 7-Zip reduces the virtual `$INSTDIR` prefix

Hosted controlled run `32654783592` completed the heavyweight Windows compilation, kept the canonical candidate source clean, and completed TEMP cleanup, but the archive Judge then failed with `Installer archive extraction must contain exactly one explicit NSIS $INSTDIR application root; found 0.` This is not an artifact, Release, Evidence or AC success; `AC-RL-12` remains `Partial`.

This result supersedes the physical-`$INSTDIR` interpretation in the immediately preceding 2026-08-24 archive-layout section, while preserving that section and its earlier `$PLUGINSDIR` misdiagnosis as history. The decisive upstream behavior is [`GetReducedName()` in 7-Zip `NsisIn.h` lines 406–444](https://github.com/ip7z/7zip/blob/f9d78aff31a5f2521ae7ddbdc97c4a8855808959/CPP/7zip/Archive/Nsis/NsisIn.h#L406-L444): after composing the NSIS prefix and item name, lines 435–440 explicitly remove the leading `$INSTDIR\\`. [`NsisHandler.cpp` lines 308–313](https://github.com/ip7z/7zip/blob/f9d78aff31a5f2521ae7ddbdc97c4a8855808959/CPP/7zip/Archive/Nsis/NsisHandler.cpp#L308-L313) exports that reduced value as `kpidPath`. Thus the hosted `7z x` extraction root is the physical application root, while the separate absolute `$PLUGINSDIR` remains a direct physical directory.

The archive Judge and public generator fixture now use that observed contract: the extraction root must hold the globally unique AMD64 `Rain.exe`; the manifest, worker and three CUDA DLLs must each be globally unique and lie exactly at `resources/whisper-backends` below that root; the payload remains an exact five-file set. Exactly one case-exact `$PLUGINSDIR` directory is required across the complete tree and it must be a direct child. The Judge does not require a physical `$INSTDIR`; application files or payload placed beneath a physical `$INSTDIR` still fail their exact-root location rules. Missing, nested, duplicate and wrong-case plugin directories, plugin/instdir Rain placement, root-external duplicate Rain/worker, payload relocation and payload supersets all have public-generator negative coverage. Size/SHA-256, duplicate, secret and hygiene scanning remain full-tree additive checks, with no recursive first-match and no unsupported plugin-file whitelist.

TDD evidence for this correction begins with the bare-root fixture RED: it produced the same `$INSTDIR`-count-zero failure as Hosted run `32654783592`. The minimal root-layout implementation made that public positive case GREEN; the focused archive/layout suite is GREEN. The complete 69-test release-generator suite was then rechecked in bounded output groups and is GREEN; `tsc --noEmit`, `harness:control`, sequential `build:e2e` then `build`, PowerShell AST, diff and `src-tauri/target`-absence checks are GREEN. Before the P2 remediation, the coordinating session ran complete `npm test` on this new diff: 97 files passed, 1 skipped; 709 tests passed, 1 skipped. A Standards re-review then found one P2 gap: an empty physical `$INSTDIR` marker, including case-variant `$instdir`, was accepted despite 7-Zip's case-insensitive prefix removal. Two public-generator fixtures first produced RED by resolving successfully; the Judge now uses a complete-tree case-insensitive zero-count check for physical `$INSTDIR` markers, and the focused layout cases are GREEN while existing Rain/payload location negatives remain. The final stable-diff complete `npm test` after that P2 remediation passed 97 files with 1 skipped and 711 tests with 1 skipped; the current complete frontend-test state is therefore GREEN. The final independent Spec and Standards re-reviews both return PASS with P0/P1/P2 empty; the preceding physical-marker P2 is closed by the case-insensitive zero-marker Judge and its two public RED→GREEN fixtures. Hosted Harness run `32658388095` then exposed one non-production portability failure at `scripts/release-artifact-generator.test.ts:922`: the production error retained its exact payload-location meaning, but PowerShell formatting inserted hosted-only line/format characters between `below the` and `extraction root`, which the old `\s+` assertion could not observe through the command-failure wrapper. The public assertion retains the complete `below the` payload-location meaning and permits only intervening whitespace or ANSI SGR sequences before `extraction root`; no production behavior, AC or Evidence tier changed. The corrected Hosted Harness is still pending. No local Cargo/Rust/Whisper/CUDA build, `harness:check`, installer/download, GPU/model/LLM execution, dispatch, commit, push, artifact, Release or Evidence action occurred for this correction. Protected PR review, Hosted Harness and a new controlled hosted run remain required before any stronger claim; `AC-RL-12` remains `Partial`.

### 2026-08-24 Controlled archive payload destination correction: Tauri resource destination is application-root relative

Protected PR #45's Hosted Harness run [`32661865608`](https://github.com/llbz510/rain/actions/runs/32661865608) is GREEN and its protected merge produced `666e1051ae9ef81df4b3291d28443d4d4bc06d75`. The subsequent controlled GPU artifact run [`32659857338`](https://github.com/llbz510/rain/actions/runs/32659857338) completed its heavyweight Windows build, kept the canonical candidate source clean, and completed invocation-owned TEMP cleanup. Its artifact Judge then failed on the obsolete expected path `resources/whisper-backends/payload-manifest.json`; no artifact upload occurred. This is not an Artifact, Evidence, Release, or AC success, and `AC-RL-12` remains `Partial`.

This supersedes only the current payload-destination conclusion in the preceding archive-layout record; its earlier `$PLUGINSDIR`/physical-`$INSTDIR` assumptions and outcomes remain preserved as audit history. The fixed evidence is Tauri CLI `v2.11.4`'s [`installer.nsi`](https://github.com/tauri-apps/tauri/blob/tauri-cli-v2.11.4/crates/tauri-bundler/src/bundle/windows/nsis/installer.nsi): after `SetOutPath $INSTDIR`, each resource is emitted with `File /a /oname={{resource destination}}`. The target-bound [`src-tauri/tauri.gpu.conf.json`](../src-tauri/tauri.gpu.conf.json) maps all five CUDA payload inputs to `whisper-backends/...`, and [`src-tauri/src/whisper_backend.rs`](../src-tauri/src/whisper_backend.rs) resolves the runtime worker through `resource_dir.join("whisper-backends")`. Together they prove the physical installed and 7-Zip-reduced application-root payload location is `whisper-backends/{payload-manifest.json,rain-whisper-cuda.exe,cublas64_12.dll,cublasLt64_12.dll,cudart64_12.dll}` rather than a synthetic `resources/` parent.

The narrow correction is on `codex/m3-tauri-resource-destination`, created from that exact protected-merge commit. `scripts/release-artifact-generator.psm1` now requires the bound install proof, logical output proof, archive manifest, worker/DLL locations and installed-tree payload directory to use application-root `whisper-backends`. `scripts/nvidia-release-evidence-contract.psm1` now requires the corresponding artifact-manifest and logical installation-proof path. The archive Judge still requires exactly one AMD64 `Rain.exe` at the extraction root, exactly one case-exact direct `$PLUGINSDIR`, no case-insensitive physical `$INSTDIR` marker, globally unique manifest/worker/three DLLs at the exact five-file payload set, and unchanged size/SHA-256, duplicate, secret and hygiene scans; it does not recursively select a first match.

TDD began RED with only unlocked public fixtures changed: the new bare-root `whisper-backends` positive generator case failed with the old `resources/whisper-backends` diagnostic; an explicit legacy `resources/whisper-backends` archive fixture was incorrectly accepted; and the NVIDIA provenance fixture returned status 1 where the new logical artifact/install proof must succeed. After the path-only production correction, the three focused generator cases are GREEN (3/3), the focused NVIDIA artifact/install/reconciliation contracts are GREEN (4/4), and the complete generator suite is GREEN (72/72). The stable-diff `npm.cmd test` run is GREEN: 99 test files passed and 1 contractually skipped live-key file; 722 tests passed and 1 skipped. It included the complete 72-case generator and complete 65-case NVIDIA contract suites, and did not reproduce the known Chinese-path child-process encoding failure. `npm.cmd exec -- tsc --noEmit`, `npm.cmd run harness:control`, strict sequential `npm.cmd run build:e2e` then `npm.cmd run build`, and PowerShell AST parsing for all 14 tracked `.ps1`/`.psm1` files are GREEN. Both builds retain only the existing Vite dynamic/static-import warnings, and the final `dist` is the ordinary production bundle. `git diff --check` and `src-tauri/target` absence are GREEN. A first independent Standards re-review returned PASS, while the independent Spec re-review found P1: `Assert-CandidateArtifactProvenance` fixed the payload-manifest path but still accepted a worker or CUDA runtime declaration at another path. New public cases first RED by accepting the legacy worker path, each legacy DLL path, unknown, duplicate and missing runtime declarations; the contract now requires the exact worker path and exactly the three named runtime files at their matching `whisper-backends/` paths. The focused new case is GREEN and the complete NVIDIA contract suite is GREEN (66/66). After this P1 correction, the current complete `npm.cmd test` run is GREEN: 99 test files passed and 1 contractually skipped; 723 tests passed and 1 skipped. `tsc --noEmit`, `harness:control`, strict sequential `build:e2e` then `build`, PowerShell AST for all 14 tracked scripts, `git diff --check`, and `src-tauri/target` absence are also GREEN. Final independent Spec review is PASS with P0/P1/P2 empty. Standards initially found and then rechecked a P2 control-document metadata gap: the active contract's update date still said 2026-08-11 despite this 2026-08-24 path revision; it now says 2026-08-24, and the final Standards review is PASS with no remaining P0/P1/P2. Protected PR, Hosted Harness and a new controlled run are still pending; no local Cargo/Rust/Whisper/CUDA build, `harness:check`, installer/download, GPU/model/LLM execution, dispatch, commit, push, artifact upload, Evidence, or Release action occurred.

### 2026-08-24 AC-VL-03 title-only search and composed sorting

This user-visible slice is in local TDD GREEN on `codex/ac-vl-03-title-search`, created from exact `origin/master` `77d2d59687282eed3bba22eaf74ca8546376528e`; it is not committed, reviewed, merged or a Release/Evidence claim. `AC-VL-03` remains the confirmed boundary: a keyword searches only the persisted Video title, trimming leading/trailing whitespace; it does not add tag, note, subtitle, full-text or fuzzy search.

- `src/models/database-videos.ts` now provides the public `queryVideos(db, { titleKeyword?, sortBy? })` interface and `src/models/database.ts` exports it. Both adapters share trimmed ASCII-case normalization for title matching and the existing three sort choices. The new query path has identical explicit tie ordering: descending `lastStudied/createdAt` then ASCII `id`, or ASCII-folded title then raw title then `id`; SQLite uses parameterized `instr(LOWER(title), LOWER($1)) > 0`, so `%`/`_` remain literal text rather than SQL wildcards, and always composes that filter with the selected `ORDER BY`. The public SQLite result is then passed through the same deterministic helper as Memory, so a driver/collation difference cannot change the returned order. Existing `listVideos` and `searchVideosByTitle` interfaces retain their prior compatibility behavior for existing callers.
- `VideoListPage` is now the single consumer of that query interface for initial loading, search/sort changes and import-triggered refresh. An empty trimmed keyword restores the selected sort. A nonempty title query with no rows displays `没有找到匹配的视频`; an actually empty library keeps the original import prompt. Query and database-initialization failures display `无法加载视频列表：<原因>` rather than becoming a false empty-library/no-match result, and a later successful query clears that error.
- New unlocked public behavior Judges are `src/__tests__/video-title-query.test.ts` (real Memory fixture plus SQLite SQL contract) and `src/__tests__/video-list-title-search.test.tsx` (production controls, trim/case/order, clear, no-match, query failure recovery and initialization failure). They first RED because `queryVideos` was absent, Memory title matching was case-sensitive and unsorted, and the page rendered every failure/no-match as the empty library. A second adapter-synonym RED proved that equal sort values retained insertion order and SQLite `%` matching did not mean the same as Memory literal matching; the explicit common tie order and `instr` contract close that gap. A third RED gives the SQLite adapter deliberately reverse-ordered mixed ASCII/non-ASCII rows and proves the public result could still inherit driver order; the result now passes through the shared helper. They are GREEN at 8/8 after the minimal implementation.
- Adjacent database/list-page regressions are GREEN: `database-videos`, deletion, local import, recovery and study tests 23/23; the remaining page consumers (URL import and task dialog) 27/27. `npm.cmd exec -- tsc --noEmit` and the pre-change `npm.cmd run harness:control` passed. No locked Harness, Cargo/Rust/Whisper/CUDA build, `harness:check`, installer, GPU, model or LLM test ran locally.

PR #44's protected `Harness` run [`32659189401`](https://github.com/llbz510/rain/actions/runs/32659189401) is completed GREEN on `6d3d396a8c655ec09e94855ead6f64fc465a8a56`; its protected merge produced the exact current baseline `77d2d59687282eed3bba22eaf74ca8546376528e`. The controlled GPU artifact run [`32659857338`](https://github.com/llbz510/rain/actions/runs/32659857338) targets that merged commit and is currently `in_progress`; it is not an artifact, Release or Evidence result. AC-VL-03 remains `Partial`: local public Memory/UI and SQLite-contract coverage is now concrete, but no real SQLite/Tauri desktop Judge or Evidence exists.

The pre-review stable-diff local frontend verification was GREEN: `npm.cmd test` passed 99 files / 719 tests with the existing one live-key file/test skipped; `npm.cmd exec -- tsc --noEmit`, `npm.cmd run harness:control`, strict sequential `npm.cmd run build:e2e` then `npm.cmd run build`, PowerShell AST parsing for all 14 tracked `.ps1`/`.psm1` files, and `git diff --check` passed. The later review remediation below changes the current diff and therefore requires a new complete frontend run before this count can be treated as current.

Independent Standards/Spec review then found no AC expansion but required four corrections before final review. A production-page delayed Promise Judge first RED because the old import `onChanged` callback used a second `refreshVideos` writer: an old-keyword import refresh could resolve after a newer query failure, clear that failure and publish stale cards. `VideoListPage` now has one query effect writer keyed by `db/sortBy/keyword/refreshRevision`; import changes only increment that revision, so every prior query is cancelled before a newer query owns the visible list/error state. The review also added a named `搜索视频标题` input, a polite live `status` for a no-match result, and a real empty-library positive Judge. The title-only fixture gives a nonmatching title a file path and persisted error containing `SIGNAL`; a temporary mutation that searched `file_path` first RED by exposing that card, then the implementation was restored. Query fixtures now prove both `%` and `_` remain literal. Focused AC-VL-03 tests are GREEN at 10/10; affected database/list-page regressions are GREEN at 50/50, with TypeScript, control-plane, diff and target-absence checks also GREEN. `AC-VL-03` stays `Partial`: real SQLite/Tauri desktop Judge and Evidence are still absent. No locked Harness, Cargo/Rust/Whisper/CUDA build, `harness:check`, installer, GPU, model or LLM operation ran.

The two complete post-remediation `npm.cmd test` attempts have the same sole environmental failure: `scripts/validate-evidence.test.ts` launches a child process that serializes this workspace path `D:\xiangmu\rain 未完成` as mojibake `D:\xiangmu\rain δ���`, then compares it to the correctly decoded parent path. Each complete run therefore reports 98 files passed, 1 failed and 1 skipped; 720 tests passed, 1 failed and 1 skipped. The isolated `scripts/validate-evidence.test.ts` rerun is GREEN at 18/18, including its Chinese-path fixture, so this is retained as a known full-suite child-process encoding instability rather than silently changing an unrelated Evidence Judge. Post-remediation `npm.cmd exec -- tsc --noEmit`, `npm.cmd run harness:control`, strict sequential `npm.cmd run build:e2e` then `npm.cmd run build`, and PowerShell AST parsing of all 14 tracked script/module files are GREEN; the two builds retain only their existing Vite dynamic/static-import warnings and leave ordinary production `dist`. `git diff --check` is GREEN and `src-tauri/target` remains absent. The current full frontend suite must not be claimed GREEN until this unrelated environmental failure is resolved or a repeatable green full run is obtained.

The final Standards P2 found that the new `VideoListPage` file-header comment overclaimed that every list fact has the `queryVideos` effect as its only writer. This was documentation-only: the committed delete path intentionally and correctly synchronously removes its own card. The comment now precisely limits the single-writer claim to query/import-refresh results and names the deletion exception. No production behavior, AC, Judge tier or verification result changed; final independent review remains pending and must not yet be reported as PASS.

The final independent Spec and Standards reviews both return `PASS`, with P0/P1/P2 empty. They confirm the public title-only query, deterministic cross-adapter ordering, literal wildcard treatment, cancellation-aware import refresh, accessible no-match state, title-field mutation proof and precise deletion-comment boundary all meet the confirmed AC without extending search scope. This conclusion does not change the known standard full-suite state: post-remediation `npm test` still has the recorded Chinese-path child-process failure, while the isolated evidence file is GREEN at 18/18. `AC-VL-03` remains `Partial`, and the protected Hosted PR gate is still pending; no real SQLite/Tauri desktop Judge, Evidence, artifact or Release claim exists.

### 2026-08-25 Controlled artifact post-build module-import correction

Protected PR [#46](https://github.com/llbz510/rain/pull/46) passed its Clean Windows Harness run [`32844901951`](https://github.com/llbz510/rain/actions/runs/32844901951) and merged through the protected path as `master@7c63d71f9a49a617d5fc34da421da97ab044cf5e`. The rerun [`32846008675`](https://github.com/llbz510/rain/actions/runs/32846008675) then completed the heavyweight hosted CUDA worker, Rain and NSIS build, but failed before artifact upload while generating the manifest from the real installer. It is not an Artifact, Evidence, Release, GPU test or AC success; `AC-RL-12` remains `Partial`.

The exact remote error was `Invoke-ReleaseEvidenceNsisInstallAndVerify is not recognized` after the workflow first imported `nvidia-release-evidence-contract.psm1` and then imported `release-artifact-generator.psm1`. A one-second local PowerShell loop first reproduced that exact import order: the verifier is exported after its module alone, but disappears after the generator's transitive `Import-Module -Force` reloads the same dependency. The public regression in `scripts/release-artifact-generator.test.ts` first RED with `Controlled workflow import sequence lost the NSIS install verifier.` The minimal correction removes only that nested `-Force`; the generator still loads the dependency, but no longer removes commands already exported to the workflow. The focused test is GREEN, the complete generator suite is GREEN (73/73), and the complete NVIDIA evidence-contract suite is GREEN (66/66). The full frontend suite is GREEN (99 files passed, 724 tests passed, 1 intentionally skipped); `tsc --noEmit`, `harness:control`, sequential `build:e2e` and production `build`, PowerShell AST for all 14 tracked scripts/modules, and `git diff --check` are also GREEN. Independent Spec and Standards reviews both return `PASS`, with P0/P1/P2 empty. A protected PR and another controlled hosted build remain pending. No local Cargo/Rust/Whisper/CUDA build, `harness:check`, real installer, GPU, model or LLM test has run for this correction.

### 2026-08-25 Launch Slice — AC-SU-05 production original subtitles

This uncommitted local Slice is on `codex/launch-su05-subtitles`, created from exact `origin/master`/HEAD `4baced9fc44c139b8a9fe20f6ddad0b0ff724776`. PR #47 is protected-merged and its Clean Windows Harness run `32851392526` is GREEN. The previously scheduled Controlled GPU Artifact Build run `32852597521` was actively cancelled by the user; cleanup succeeded and no manifest, core/control artifact, build record or launcher was generated/uploaded. RL/controlled GPU work is user-paused and must not be resumed without explicit user direction.

`AC-SU-05` remains `Partial`. The only product change is the default follow production path: `src/pages/StudyInterface.tsx` derives the original subtitle through the exact half-open interval `[startTime,endTime)` and passes it to `VideoZone`; it deliberately does not use the page's existing nearest `currentSentence` helper, which remains QuickActions-only. `src/ui/components/video.tsx` keeps its single existing dynamic `字幕 ON/OFF` control and session-only `subtitleOn` state (`aria-pressed`); no localStorage/SQLite/second subtitle state, translated-subtitle control, `translationOn`, translation block or QuickActions behavior changed. `unloadVideo` continues not to reset this app-session setting.

TDD RED first changed only unlocked `src/__tests__/study-playback.test.tsx` through production `StudyInterface`: the targeted Vitest run had 12 tests, 11 passed and 1 failed because the production path did not provide the requested original-subtitle control. GREEN makes the same targeted run pass 12/12. `npm.cmd run build` passed (only existing Vite dynamic/static-import warnings), `npm.cmd run harness:control` passed, and `git diff --check` passed. No Cargo/Rust/Whisper/CUDA test or build, `harness:check`, installer/GPU/model/LLM execution, remote workflow dispatch, artifact, Evidence, signing, license or Release action occurred.

`docs/development/launch-feature-audit.md` is the Active production-path/blocker ledger: it contains all 89 current Confirmed AC rows (excluding Superseded `AC-RL-07`) and the 23 Proposed local surfaces. It does not define AC semantics, Judges, tiers or completion. AC-SU-05 still needs real media/Tauri/Desktop/Visual/screenshot Evidence; jsdom and fake screenshots cannot close that requirement. The stable diff is ready for independent Spec and Standards review, not committed or merged.

The first independent double review found and this still-uncommitted diff corrects its blocking findings; this is not a final review PASS. P1: locked M06's public dynamic text contract remains one `字幕 ON`/`字幕 OFF` button with `aria-pressed`; the briefly introduced hidden companion button and `exposeOriginalSubtitleLabel` prop were removed because two interactive entries for one subtitle state violate the single-control/keyboard/screen-reader boundary. The production Judge now uses the existing subtitle button name, asserts exactly one matching subtitle control and asserts no translation control. P2: the AC-SU-05 Judge moves to `currentTime=3` before closing, proves immediate overlay disappearance while the transcript remains, reopens immediately to the same sentence, then closes, calls the production back action, explicitly `unmount()`s, seeds no `subtitleOn`, and newly mounts `StudyInterface` to prove `aria-pressed=false` before reopening at 3 seconds. The revised required Judge wording only calls this a “重开恢复” proof after the real unmount/remount path. Post-correction verification is GREEN: combined `study-playback` + locked M06 is 17/17, production build and control-plane validation pass, and `git diff --check` is clean. No prohibited operation occurred.

Standards re-review P2 then found an audit wording inconsistency: the old preface incorrectly implied only cells literally prefixed `阻断：` could block completion, while historical/current-target, RL, PF and AR rows also carry unclosed Required Evidence. The preface now states the mechanical shared rule: every unclosed Required Evidence item is a blocker unless its row explicitly says no requirement, no new requirement, supplement-only, or conditional future replay; `阻断：` is emphasis only. This documentation correction is not a final double-review PASS.

Final Spec review found one remaining audit-summary P2: the LV/ST ledger's historical/current-target list omitted `AC-LV-21` even though its row correctly records a target NVIDIA Release Evidence Gap. The ledger now explicitly includes `LV-21` as that user-paused Release Evidence blocker; this is a documentation-only correction and not a final review PASS.

### 2026-08-26 AC-SU-05 final stable-diff review handoff

The final same-diff independent Spec and Standards reviews both return `PASS`, with no P0/P1/P2. The review corrections retained locked M06 compatibility by preserving the single dynamic `字幕 ON/OFF` control; removed the briefly introduced duplicate hidden companion so one `subtitleOn` has one keyboard/screen-reader control; and strengthened the unlocked production Judge to prove exact half-open subtitle timing, close/reopen, and a real return → `unmount()` → fresh same-session mount recovery. The Active launch audit now uses one mechanical blocker rule for all unclosed Required Evidence (unless explicitly no requirement/supplement-only/conditional) and explicitly includes the user-paused `AC-LV-21` target NVIDIA Release Evidence blocker.
...
The other two failures are in unchanged `scripts/nvidia-release-evidence.test.ts`: an empty-value failure and a 5-second timeout. They are not attributed to this diff and are not modified or targeted here; no GPU, Release, installer, model, LLM, Cargo/Rust, manual workflow dispatch or rerun is authorized. This follow-up only repairs the deterministic AC-ST-03 selector regression. Its allowed verification is GREEN: the navigation/playback/locked-M06 focused Vitest set passes 22/22; production build, control-plane validation and `git diff --check` pass. The final same-diff independent Spec and Standards reviews both return `PASS`, with no P0/P1/P2. It was subsequently merged through protected PR #48 as `master@54c6f2422f43d8b1cacff6b314ffab8cc4ebc9a5`; its automatic Clean Windows Harness run `32928014936` is GREEN.

### 2026-08-26 Launch Slice — AC-SU-03 unsaved Note draft across right-panel Tabs

PR #49 was protected-merged at `2026-08-26T08:41:30Z` as `master@7d6d8cce50906c0b4c252f233a7b18f92b326691`; its automatic Clean Windows Harness run `32929922305` is `SUCCESS`. The merged Slice was `codex/launch-su03-note-draft`, created from the PR #48 baseline. It changes only `AC-SU-03`'s Note-draft ownership/remount gap: `StudyInterface` keeps the existing `NotesPanel` mounted behind the platform-native `hidden` attribute while AI is active. That preserves the existing `NoteItem` local textarea draft without writing it to SQLite, localStorage, or a second business state; the hidden panel is removed from layout, accessibility and keyboard navigation. The existing explicit save path and Note persistence semantics are unchanged.

TDD RED first added the unlocked production `StudyInterface` + memory-DB path in `src/__tests__/study-notes.test.tsx`: create and save a Note, edit it to an unsaved draft, switch AI → 随记, assert the draft and old database value, then save and assert the database update. The targeted RED run had 4 tests: 3 passed and 1 failed because the conditional Tab composition remounted the editor with `Saved note.` rather than retaining `Unsaved draft.`. GREEN replaces only that NotesPanel conditional unmount with a native-hidden retained mount. Review follow-up P2 moves this use case out of the `AC-ST-06` describe into its own `AC-SU-03` describe, starts with an already persisted Note while AI is initially active, and observes the actual memory adapter's `replaceTable` writes: initial hidden mount writes 0; after clearing the first explicit-save count, AI/随记 switching writes 0 and the later explicit save writes exactly 1. It also proves the native `hidden` wrapper, default a11y absence of the named Note editor/save control, and one Tab from the AI input not entering that hidden panel. Its added accessibility RED was 3/4 pass because the existing Note textarea had no public name; GREEN adds only `aria-label="随记内容"`, not another control or state. The focused `study-notes`, `study-layout`, and locked M08 run is GREEN at 9/9; `npm.cmd run build` is GREEN with only existing Vite dynamic/static-import warnings; `npm.cmd run harness:control` and `git diff --check` are GREEN. Final same-diff Spec and Standards reviews are both `PASS`, with no P0/P1/P2/P3.
...
### 2026-08-26 Launch Slice — AC-SU-01 two-row catalog

This Slice was developed on `codex/launch-su01-two-row-catalog`, created from exact `master@1a182c57a886202ced32393c25a2b32d6e5f8d14`. `AC-SU-01` remains `Partial`. The only product change is the production `StudyInterface → CatalogBar` route: `CatalogBar` groups existing `chapter`/`section` nodes into its top horizontal row and `paragraph` nodes into its bottom horizontal row. Both rows are flex no-wrap and horizontally scrollable; every existing node click continues through `onSeek(node.startTime)` and `StudyInterface`'s existing handler writes the real Store `playPosition`. No automatic current-item positioning, pause/follow policy, edge fade, motion/reduced-motion behavior, tree editing, Desktop Evidence, Cargo/Rust/Whisper/CUDA, installer/GPU/model/LLM, full `harness:check`, or remote workflow action was introduced or run.

TDD RED first added an unlocked production `StudyInterface` Judge in `src/__tests__/study-navigation.test.tsx`, using only CatalogBar DOM/public clicks and real Store state. `npm.cmd exec -- vitest run src/__tests__/study-navigation.test.tsx` reported 5/6 passing and failed because the requested structure row was absent (`null`). GREEN adds only the two CatalogBar rows. The same targeted Judge is GREEN at 6/6; the navigation Judge plus read-only locked M05 is GREEN at 15/15; `npm.cmd exec -- tsc --noEmit`, `npm.cmd run build` (only existing Vite dynamic/static import warnings), `npm.cmd run harness:control`, and `git diff --check` are GREEN. `docs/development/harness-coverage.md` and `launch-feature-audit.md` now state this exact local Judge without closing the remaining Strong + Desktop Evidence gap.

First independent Spec/Standards review found two P2 Judge gaps without expanding AC-SU-01: the original Judge did not prove `overflowX: auto`, each node's `flex: 0 0 auto`/`whiteSpace: nowrap`, or that the structure row is physically before the paragraph row. The same public production-page Judge now asserts those row and node styles for both levels and verifies `catalog.children[0]` is the structure row and `catalog.children[1]` the paragraph row. Mutation proof is recorded: changing `overflowX` to `hidden`, changing `flex` to `1 1 auto`, changing `whiteSpace` to `normal`, and swapping the two production rows each made the targeted run RED at 5/6 with only AC-SU-01 failing; every mutation was immediately restored. The final navigation plus read-only locked M05 run is GREEN at 15/15, with `tsc --noEmit`, ordinary `build`, and `harness:control` GREEN again. This strengthens only the existing local structural/scrollability proof; automatic positioning, pause/follow policy, edge fade, AC-SU-02 motion/reduced-motion and Strong + Desktop Evidence remain unimplemented or unsigned.

Final independent Spec and Standards reviews of this same diff both return `PASS`, with P0/P1/P2 empty. This review result does not change the remaining AC-SU-01 Evidence gap or authorize any omitted behavior.

### 2026-08-27 Launch Slice — AC-SU-01 playback catalog follow

This uncommitted local Slice is on `codex/launch-su01-playback-follow`, created from exact `origin/master`/HEAD `7dd3be0bfc4e385ff98ed12a72598cdb539a5fa9`. `AC-SU-01` remains `Partial`. The only production change is in `CatalogBar`: it derives each row's current node from the existing Store `playPosition` using the same half-open interval convention, deterministically preferring a later-started node and then the deeper child section over its parent at equal boundaries. During existing Store `isPlaying=true`, it follows the top-row chapter/section node and bottom-row paragraph node with `scrollIntoView({ block: 'nearest', inline: 'center' })`; a missing API is optional and does not throw. It tracks only the previously followed node IDs in component refs; it adds no time state, animation, edge fade, or reduced-motion behavior. While paused, it resets those follow markers without scrolling, so media time updates and public catalog seeks leave the user's horizontal position under manual control; the next play immediately re-centers the current two row items.

TDD first added an unlocked production `StudyInterface → CatalogBar` Judge in `src/__tests__/study-navigation.test.tsx`. The first attempted command was setup-blocked because this worktree lacked frontend dependencies; after `npm install`, `npm.cmd exec -- vitest run src/__tests__/study-navigation.test.tsx` produced the actual RED: 6/7 passed and the new AC-SU-01 Judge failed because the expected current `Section two` and `Paragraph two` elements had no `scrollIntoView` calls. A nested chapter/section fixture then exposed a second RED (only the paragraph followed), so current-node selection was refined to prefer the latest active start time and correctly follow an entered child section over its still-active parent chapter. The same focused command is GREEN at 7/7. The Judge drives real video play/pause/timeupdate events and a public catalog click, asserts follow on nested section/paragraph and chapter/paragraph boundary changes, and asserts no forced scroll after pause. It does not sign off long-directory visual visibility, edge fades, Strong coverage, Desktop Evidence, or any AC-SU-02 motion/reduced-motion work.

Double-review follow-up added production Judge coverage for resume-to-current re-centering, exact `inline: 'center'`, equal-boundary parent/child precedence and missing `scrollIntoView`. Its initial focused RED was 6/8: the top row followed same-boundary `Chapter two` rather than child `Section two`, and deleting `Element.prototype.scrollIntoView` threw `TypeError`; the resume and centered-call assertions were part of that same public test path. After the production correction, a transient 7/8 assertion failure identified an existing SideTree `block: 'nearest'` call co-occurring on resume; the Judge now associates exact center options to the CatalogBar elements themselves rather than treating that independent call as a product failure. Final allowed verification is GREEN: `npm.cmd exec -- vitest run src/__tests__/study-navigation.test.tsx harness/m05-catalog-component.test.tsx` passes 17/17; `npm.cmd exec -- tsc --noEmit` passes; `npm.cmd run build` passes with only the existing Vite dynamic/static-import warnings; `npm.cmd run harness:control` passes; and `git diff --check` passes. No locked Harness or Rust test changes, Cargo/Rust/Whisper/CUDA build, `harness:check`, installer/GPU/model/LLM/Evidence action, remote workflow, artifact, signing, license, or Release operation has occurred. `node_modules/` is newly ignored local dependency setup and is not a source change.

Spec re-review then found one P2 Judge precision gap: the paused path first issued a media update to 0 and then clicked the `startTime=0` chapter, so it did not prove a changing public CatalogBar seek. The unlocked production Judge now pauses at 20, drives real media `timeupdate` to 10, asserts the Store is 10 and has not scrolled, then clicks the public `Chapter one` CatalogBar node and proves `playPosition` changes to 0 without scroll before resuming and centering the new current pair. This is test/control-document-only; production source is unchanged, so build was not rerun. The focused navigation plus locked M05 test is GREEN at 17/17; `tsc --noEmit`, `harness:control`, and `git diff --check` are GREEN.

The completed diff's final independent Spec and Standards reviews both return `PASS`, with P0/P1/P2 empty. Spec confirms the production `StudyInterface → CatalogBar` path follows the current two-row items only while playing, re-centers the newly sought current items on resume, uses the exact center options, deterministically prefers a same-boundary child section, and leaves absent browser scroll APIs safe without introducing another time fact or SU-02 behavior. Standards confirms the Judge observes that public media/Store/catalog route, separates the independent SideTree call from CatalogBar parameter assertions, proves a real paused `10 → 0` public seek, leaves locked Harness untouched, and keeps the coverage/audit claims bounded. Final local checks are GREEN: focused navigation plus M05 17/17, `tsc --noEmit`, `harness:control`, and `git diff --check`. This review result does not upgrade `AC-SU-01` from `Partial` and does not close its required Desktop Evidence, long-directory real visible-scroll, or edge-fade gaps.

### 2026-08-27 Launch Slice — AC-SU-01 catalog edge fades

This uncommitted local Slice is on `codex/launch-su01-edge-fades`, created from exact `origin/master`/HEAD `fc51bfea327acc92a58ab1c1e517daf2bddec703`. `AC-SU-01` remains `Partial`. The only product change is the production `StudyInterface → CatalogBar` route: each of its existing horizontal rows has a scroll owner and a positioned visual shell. The shell renders a left or right gradient only when that row's actual `scrollWidth - clientWidth` is positive and `scrollLeft` proves content remains in that direction. Those hints are non-interactive (`pointerEvents: none`) and `aria-hidden`; they update on initial layout, catalog-node changes, user scrolling and window resize, with the resize listener removed on cleanup. No playback/time state was introduced or changed, and the preceding follow policy and AC-SU-02 animation/reduced-motion behavior were not expanded.

TDD first added an unlocked production `StudyInterface → CatalogBar` long-directory Judge in `src/__tests__/study-navigation.test.tsx`, with controlled DOM `clientWidth`/`scrollWidth`/`scrollLeft` and real scroll/resize events. `npm.cmd exec -- vitest run src/__tests__/study-navigation.test.tsx` was RED at 8/9 because `catalog-fade-right-structure` was absent. GREEN adds only the CatalogBar fade shell/geometry update logic. The same focused Judge is GREEN at 9/9: it proves both rows start with only their right gradient, moving to their ends shows only the corresponding left gradient, gradients use the intended actual CSS directions, and no-overflow removes both; it also proves node changes cause geometry recomputation. It does not supply a real long-directory desktop visible-scroll or visual Evidence result. Final permitted verification is GREEN: focused navigation plus read-only M05 18/18, `npm.cmd exec -- tsc --noEmit`, `npm.cmd run build` (only existing Vite dynamic/static-import warnings), `npm.cmd run harness:control`, and `git diff --check`; no locked Harness/Rust change, Cargo/Rust/Whisper/CUDA, full `harness:check`, installer/GPU/model/LLM/Evidence action or remote workflow was run.

Double-review follow-up closed five local Judge/implementation gaps without changing the AC boundary. A second public-path RED (`npm.cmd exec -- vitest run src/__tests__/study-navigation.test.tsx`) is 8/9: the right fade used `linear-gradient(to left, var(--color-surface), transparent)` rather than the actual CatalogBar `--color-bg` opaque endpoint. The expanded same Judge locks that color and both directions, requires both production rows to show both fades at midpoint, proves 0.25/199.75 fractional positions around a 200px range normalize to the intended start/end, and gives geometry only to `[data-catalog-scroll-row]` so assigning the ref to its shell fails. It also spies real geometry reads and window resize listener calls: a Store `playPosition` change must not cause remeasurement or listener rebinding. GREEN changes only CatalogBar internals: stable `useMemo` row arrays, a node-triggered layout measurement separate from a once-mounted resize listener, unchanged-fade bailout, and 0.5px bounded epsilon/normalization. The repeated focused Judge is GREEN at 9/9; focused navigation plus read-only M05 is GREEN at 18/18, and `tsc --noEmit`, ordinary `build` (only existing Vite dynamic/static-import warnings), `harness:control`, and `git diff --check` are GREEN.

The final independent Spec and Standards reviews both return `PASS`, with P0/P1/P2 empty. The last focused mutation made only the production left-fade opaque endpoint `--color-surface`; `npm.cmd exec -- vitest run src/__tests__/study-navigation.test.tsx` then correctly went RED at 8/9 on the exact required `linear-gradient(to right, var(--color-bg), transparent)` assertion, before the implementation was restored and the same command returned GREEN at 9/9. The final review accepts the production two-row geometry/fade route and its bounded public Judge without expanding scope. `AC-SU-01` remains `Partial`: Desktop Evidence, Visual Evidence and real long-directory desktop visible-scroll are not closed.

### 2026-08-27 Launch Slice — AC-SU-02 structure catalog switch feedback

This uncommitted local Slice is on `codex/launch-su02-structure-slide`, created from exact `origin/master`/HEAD `7ce27f37a07e9e778fd8dd724c4b0075f972ddcc`. It advances only the bounded production `StudyInterface → CatalogBar` path: the existing Store `playPosition` continues to be the sole timing fact. When its derived current chapter/section changes, the structure row receives a forward or backward 200ms horizontal feedback animation; the current node updates immediately. Same-level paragraph changes do not animate. When the browser reports `prefers-reduced-motion: reduce`, the current structure node still updates immediately while the structure row has no animation or displacement. This adds no persisted/current-time state, general animation framework, change to playback follow policy, or new behavior outside `AC-SU-02`.

TDD RED first added two unlocked production-page Judges in `src/__tests__/study-navigation.test.tsx`, driven only through real media `timeupdate` and public CatalogBar click. Before the implementation, `npm.cmd exec -- vitest run src/__tests__/study-navigation.test.tsx` was RED at 9/11: both new `AC-SU-02` tests failed because no current-node DOM state existed. GREEN adds the CatalogBar-only motion preference listener, deterministic structure direction and scoped keyframes. The same focused run is GREEN at 11/11. The combined production navigation test plus read-only locked M05 Harness is GREEN at 20/20, and `npm.cmd exec -- tsc --noEmit` is GREEN. No locked Harness or Rust test was modified; no Cargo/Rust/Whisper/CUDA work, `harness:check`, Desktop E2E, installer/GPU/model/LLM/Evidence action or remote workflow was run.

`AC-SU-02` remains `Partial`: its Strong Judge tier and Desktop/Visual Evidence remain required and unsigned. This Slice does not claim a real desktop animation observation, a visual review, broader catalog-content transition behavior, or completion of the whole AC.

The first independent dual review of this uncommitted diff returned `FAIL`: no P0 was reported, but P1 found that repeated same-direction structure changes reused the same animation name and that equal-start parent/child boundary direction followed node order rather than time direction. P2 required the existing `--anim-base` token to be the sole duration fact, a Judge for real ± horizontal displacement and repeat triggering, plus a runtime MediaQueryList change/unmount cleanup Judge. No contract or scope change was authorized by these findings.

The corrective TDD RED expanded the same unlocked production-page seam to 12 tests. Before GREEN, the focused run failed on the absent per-switch animation identity and reported the equal-start child-to-parent forward crossing as `backward`. GREEN keeps the existing scroll owner mounted and preserves its manual `scrollLeft`, but alternates the bounded forward/backward A/B keyframes for each structure switch. Direction now uses start time, then end-time boundary before a stable final ordering fallback, without a second Store/persisted time value. Animation duration is now `var(--anim-base)` only; the row supplies the actual forward/backward ±12px CSS offset. The motion-preference Judge triggers the registered `change` listener while mounted, proves immediate cancellation, removes the preference and proves the next switch animates, then proves unmount removes that exact listener. The corrective focused run is GREEN at 12/12 and `npm.cmd exec -- tsc --noEmit` is GREEN. The required locked-M05/read-only, ordinary build and control-plane rechecks remain pending in this note until their commands are run; Desktop/Visual Evidence remains unclosed.

The second independent dual review also returned `FAIL`: no P0 was reported. P1 correctly found the interval-order direction heuristic wrong for a legal nested `parent[0,20]/child[5,10]` transition (`6 → 11` playback must be forward and `11 → 6` user seek must be backward). P1/P2 also required a mutation-sensitive Judge that reads the actual production stylesheet, rather than merely observing the inline animation name. No evidence tier or product scope changed.

The second corrective RED added the unlocked public StudyInterface media path for that nested playback/seek fixture and an equal-sample no-switch check. The first focused run was RED at 12/14: the nesting crossing was incorrectly `backward`, and the first attempt to import CSS as a Vite raw asset produced an empty test payload. GREEN uses a component-local ref only as the previous observed `playPosition` sample for the next animation direction; it is neither Store state nor persistent state, cannot be written externally, and does not determine current node/progress. Every `playPosition` update refreshes the sample. Node interval/ID ordering remains only a stable fallback when a structure-data update changes the current node at an equal time. The Judge now reads the real `src/index.css` file and requires all four restartable A/B keyframes, the exact `translateX(var(--catalog-structure-slide-offset))` from state, `translateX(0)` to state, and the sole `--anim-base: 200ms` duration token. Mutation proof: renaming `catalog-structure-slide-forward-a` and independently changing its `from` transform to `translateY` each made that CSS Judge RED; both mutations were immediately restored. The final focused verification and remaining permitted checks are recorded only after their commands complete. `AC-SU-02` remains `Partial`; Strong plus real Desktop/Visual Evidence is still absent.

Final permitted verification for the corrected diff is GREEN: `npm.cmd exec -- vitest run src/__tests__/study-navigation.test.tsx` passes 14/14; adding read-only locked M05 and M13 passes 25/25; `npm.cmd exec -- tsc --noEmit`, ordinary `npm.cmd run build` (only the existing Vite dynamic/static-import warnings), `npm.cmd run harness:control`, and `git diff --check` pass. No locked Harness was edited and no `harness:check`, Rust/Cargo/Whisper/CUDA work, Desktop E2E, installer/GPU/model/LLM/Evidence action, remote workflow, commit or push occurred.

The third independent dual review returned `FAIL`, again without changing the bounded AC. P1 found that a real `timeupdate` inside the same current structure node (`11 → 11.05`) cleared the just-started animation even though it must only refresh the adjacent-sample reference. P2 found the four directional keyframes duplicated the same visual contract, and required the production CSS Judge to reject duplicate same-name rules and later transform overrides.

The third corrective RED first expanded the public real-media Judge with the `11 → 11.05` regression: it retains the structure switch direction, animation identity, inline animation name and style while the sole Store `playPosition` advances. The focused run was RED at 11/14 because that update changed the switch to `none`; the CSS/restart expectations were also RED while the production source still exposed four directional keyframes. GREEN continues to refresh the component-local previous-sample ref on every Store update, but clears motion only for an initial/no-current state or reduced-motion; a same-current-node sample leaves the active switch intact. It replaces the four duplicate directional names with exactly two restartable `catalog-structure-slide-a`/`-b` keyframes, while the row's signed `--catalog-structure-slide-offset` remains the only direction carrier. The actual production CSS Judge now requires only those two names once each and exactly one precise `transform` in both from/to blocks. Mutation proof appended a second `catalog-structure-slide-a` rule (RED) and independently added a second from-block transform (RED); each mutation was immediately restored. This remains a local DOM/CSS Judge: `AC-SU-02` is still `Partial`, with Strong plus real Desktop/Visual Evidence unclosed.

Third corrective verification is GREEN: `npm.cmd exec -- vitest run src/__tests__/study-navigation.test.tsx` passes 14/14; adding read-only locked M05 and M13 passes 25/25; `npm.cmd exec -- tsc --noEmit`, ordinary `npm.cmd run build` (only existing Vite dynamic/static-import warnings), `npm.cmd run harness:control`, and `git diff --check` pass. No locked Harness was edited and no `harness:check`, Rust/Cargo/Whisper/CUDA work, Desktop E2E, installer/GPU/model/LLM/Evidence action, remote workflow, commit or push occurred.

The fourth independent dual review returned `FAIL` with P2 only. It found a render-to-effect window in the reduced-motion hook: the initial `matchMedia` snapshot could be false while the MediaQueryList obtained for subscription was already true, without any `change` event. It also narrowed the CSS Judge requirement: only `catalog-structure-slide-*` rules belong to this AC, so unrelated legitimate production keyframes must not make the target contract fail.

The fourth corrective RED added an unlocked public production-page fixture whose initial `matchMedia` snapshot is false and whose subscribed MediaQueryList is true without dispatching `change`; the first focused run was RED at 14/15 because a subsequent real media structure switch incorrectly animated forward. GREEN registers the existing listener and immediately synchronizes its actual MediaQueryList `matches` value, closing that window while retaining the existing runtime-change and exact-listener cleanup Judge. The CSS helper now extracts only `catalog-structure-slide-*` rules before requiring exactly A/B, once each, with the existing exact transforms and duration token. Its inline unrelated-keyframe fixture remains accepted; a fourth-round duplicate target-A mutation is RED and was immediately restored. Coverage and launch audit remain conservatively `Partial`; Strong plus real Desktop/Visual Evidence is still absent.

Fourth corrective verification is GREEN: `npm.cmd exec -- vitest run src/__tests__/study-navigation.test.tsx` passes 15/15; adding read-only locked M05 and M13 passes 26/26; `npm.cmd exec -- tsc --noEmit`, ordinary `npm.cmd run build` (only existing Vite dynamic/static-import warnings), `npm.cmd run harness:control`, and `git diff --check` pass. No locked Harness was edited and no `harness:check`, Rust/Cargo/Whisper/CUDA work, Desktop E2E, installer/GPU/model/LLM/Evidence action, remote workflow, commit or push occurred.

The fifth independent dual review returned `FAIL` with one P2 in the CSS Judge. Its target-rule scanner first required a full `from`/`to` body, so a later same-name target declaration with a different body shape could escape the uniqueness check. The corrective Judge now uses a permissive declaration-header scan for every `@keyframes catalog-structure-slide-*` occurrence and requires that target-name sequence to be exactly A/B once each before separately applying the strict from/to transform contract to those unique rules. The unrelated-keyframe fixture stays GREEN. Mutation proof appended a later same-name A rule using `0%`/`100%` translateY (RED on the header scan), then changed the original A body itself to that `0%`/`100%` shape (RED because strict parsing finds no valid A rule); both were immediately restored. This changes no production behavior or evidence tier: `AC-SU-02` remains `Partial`, with Strong and real Desktop/Visual Evidence unclosed.

Fifth corrective verification is GREEN: `npm.cmd exec -- vitest run src/__tests__/study-navigation.test.tsx` passes 15/15; adding read-only locked M05 and M13 passes 26/26; `npm.cmd exec -- tsc --noEmit`, ordinary `npm.cmd run build` (only existing Vite dynamic/static-import warnings), `npm.cmd run harness:control`, and `git diff --check` pass. No locked Harness was edited and no `harness:check`, Rust/Cargo/Whisper/CUDA work, Desktop E2E, installer/GPU/model/LLM/Evidence action, remote workflow, commit or push occurred.

The sixth independent review records Spec `PASS` and Standards `FAIL` with one P2: the source-file CSS Judge still used regular expressions rather than browser CSS semantics. The replacement Judge injects the real production `src/index.css` into a temporary document `<style>`, reads `CSSStyleSheet.cssRules`, filters `CSSKeyframesRule` by the catalog prefix, and reads each nested keyframe's `keyText` and computed declaration `style.transform`. It removes the temporary style in `finally`, and the Judge proves that cleanup after both the production and unrelated-keyframe fixture parses. The only permitted target names remain A/B once each; their lone `from`/`to` keyframes must expose the exact expected transforms. Mutation proof appended a legal later `@keyframes catalog-structure-slide-a` with `0%`/`100%` translateY, which is RED through CSSOM target-name duplication; it was immediately restored. This is a Judge-only correction: `AC-SU-02` remains `Partial`, and Strong plus real Desktop/Visual Evidence remains unclosed.

Sixth corrective verification is GREEN: `npm.cmd exec -- vitest run src/__tests__/study-navigation.test.tsx` passes 15/15; adding read-only locked M05 and M13 passes 26/26; `npm.cmd exec -- tsc --noEmit`, ordinary `npm.cmd run build` (only existing Vite dynamic/static-import warnings), `npm.cmd run harness:control`, and `git diff --check` pass. No locked Harness was edited and no `harness:check`, Rust/Cargo/Whisper/CUDA work, Desktop E2E, installer/GPU/model/LLM/Evidence action, remote workflow, commit or push occurred.

The seventh independent Spec and Standards reviews both returned `FAIL` with P2-only CSSOM Judge gaps. The first found that a target keyframe nested in a CSS grouping rule such as `@media` was invisible to the top-level traversal. The second found that the Judge allowed extra keyframe stops alongside otherwise valid `from`/`to` transforms. GREEN now recursively traverses every non-keyframe CSS rule exposing `cssRules` (media/supports/layer and equivalent grouping rules), collecting `CSSKeyframesRule` before deliberately stopping rather than descending into its child frames. The unrelated nested-keyframe fixture remains GREEN and temporary-style cleanup remains asserted. Each unique A/B rule now has exactly two child `CSSKeyframeRule` entries, with the complete ordered keyText sequence exactly `from`, `to`, before its exact transforms are asserted. Mutation proof added a nested `@media` target-A `0%`/`100%` translateY rule (RED through duplicate target collection) and independently appended `0%` and `50%` translateY stops to original A (RED through the exact full-frame list); both were immediately restored. This remains only local semantic CSS/DOM coverage: `AC-SU-02` stays `Partial`, with Strong and real Desktop/Visual Evidence unclosed.
...
### 2026-08-28 PR #55 second Clean Windows Harness failure record

The second automatic Harness run `33148159353`/job `98773732287` for PR #55 HEAD `6c5f8ec` also ended in `failure`. The clean Windows log again reports this Slice's `src/__tests__/study-navigation.test.tsx` passing 15/15. Its `npm test` total is 736 passed, 1 failed and 1 skipped; the sole failure is `scripts/nvidia-release-evidence.test.ts`, `rejects provenance whose CUDA worker or runtime declarations leave the canonical whisper-backends payload set`, where seven status values are `null` rather than the expected `1`. The cause remains unestablished and is not called a flake. As `npm test` failed, the workflow did not enter `build:e2e`, ordinary build, or Rust testing. No local run of that target occurred and no Release/GPU code was changed. Once this record is committed, a normal PR push will trigger a third automatic Clean Windows Harness judgment. If the same class of failure recurs, stop the documentation-push loop and report the resulting blocker rather than creating further documentation-only pushes.

### 2026-08-29 Launch Slice — AC-VL-02 视频列表排序

此 Slice 位于 `codex/launch-vl02-sort-contract`，创建自精确 `origin/master` / `de8b5653feb937de4d2b174f8fae65a821d9738e`。它只收口 Confirmed `AC-VL-02` 与 `DEC-PRD-058` 的公开排序合同：用户打开列表时默认按最近学习排序，并可在具名控件中切换“最近学习 / 导入时间 / 名称”；每一档的并列结果稳定地按 ID 排序。范围外：标题搜索（AC-VL-03）、列表组合/任务入口（AC-VL-04）、视觉/桌面/Evidence、导入、删除、数据库 schema 与 Rust。

Owner 保持 `src/models/database-videos.ts` 的公开 `queryVideos` 与 `src/pages/VideoListPage.tsx` 的 query effect/排序控件。新增未锁定 `src/__tests__/video-list-sorting.test.tsx` 通过公开 seam 使用同一 fixture 的真实 Memory adapter、故意无序的 SQLite adapter，以及生产 `VideoListPage`：锁定默认最近学习、最近学习/导入时间/名称三档和完整稳定 ID 顺序。fixture 的 `a-tie`/`z-tie` 同时拥有相同 `lastStudiedAt`、`createdAt` 与 `title`，因此每一档都实际裁判 ID tie-breaker。

TDD RED 的单文件结果为 2 个测试中 1 个失败：生产页面可见选项为“创建时间”，而 Confirmed 合同要求“导入时间”；双 adapter 三档排序 Judge 已在 RED 中通过。最小 GREEN 只把 `VideoListPage` 的 `createdAt` option 文案改为“导入时间”，未修改 `queryVideos` 或任何排序逻辑；单文件随后 GREEN 2/2。改动文件仅为 `src/pages/VideoListPage.tsx`、`src/__tests__/video-list-sorting.test.tsx`、本状态文件、coverage 与 launch audit；没有修改 locked `harness/`、`src-tauri/tests/`、Rust、外部 workflow 或 Evidence。

Required Strong 已由本地公开双 adapter/生产页面行为覆盖；该 AC 不要求 Desktop、Visual 或 Evidence。基线为 `de8b5653feb937de4d2b174f8fae65a821d9738e`；独立 Spec 与 Standards 双审均为 PASS，二轴 P0/P1/P2 均为空，且各自复跑新 Judge 为 GREEN 2/2。无本 AC 必需 Evidence；仍未运行 `harness:check`、Cargo/Rust、Desktop 或 Release，受保护 PR 的自动 `Clean Windows Harness` 与合并由后续流程裁判。最终允许验证：定向列表回归为 7 files / 33 tests GREEN；`tsc --noEmit`、普通 `npm run build`（保留既有动态/静态 import chunking warnings，production E2E isolation 扫描通过）、`npm run harness:control` 与 `git diff --check` 均 GREEN。初次在默认本机 code page 936 下运行完整测试时，范围外 Git archive/provenance 用例因 `C:/` 的 dubious ownership 出现环境失败；没有修改测试或实现。随后仅在单个临时 PowerShell 进程切至 code page 65001、设置进程级 `safe.directory=*` 后，以 `--maxWorkers=1` 复验：`scripts/validate-evidence.test.ts` GREEN 18/18，完整 `npm.cmd test -- --maxWorkers=1` 为 100 passed files + 1 skipped file、739 passed tests + 1 skipped，exit 0。本 Slice 的新增 `video-list-sorting.test.tsx` 为 GREEN。finally 已恢复 code page、移除三个 `GIT_CONFIG_*` 环境变量，且未写入任何 Git config；此环境处理未放宽或归因于 AC-VL-02。本 Slice 不运行 `harness:check`、Cargo/Rust、Desktop E2E、安装包、GPU、模型、LLM 或 Evidence。

### 2026-08-29 Launch Slice — AC-VL-03 标题搜索

此 Slice 位于 `codex/launch-vl03-search-contract`，从精确 `origin/master` / `9acaf59ab7b0dc907115770b9380946eb4326f19` 创建。它只收口 Confirmed `AC-VL-03` 与 `DEC-PRD-059` 的 Strong 公开行为合同：关键词只匹配持久化 Video title，输入 trim、ASCII 大小写不敏感；用户清空搜索后当前选择的排序继续生效；无匹配、空库和数据库失败保持可区分。范围外：标签、笔记/字幕全文、模糊语义、`AC-VL-04`、Desktop/Visual/Evidence、导入、删除、Rust、Release/GPU。

Owner 保持 `src/models/database-videos.ts` 的公开 `queryVideos` 与 `src/pages/VideoListPage.tsx` 的 query effect/搜索与排序控件。新增未锁定 `src/__tests__/video-list-title-search.test.tsx` Judge 使用同一 fixture 贯通真实 Memory adapter、故意无序的 SQLite public adapter 和生产 `VideoListPage`，锁定标题-only 结果、`  SIGNAL  ` 的 trim/ASCII case-insensitive 行为、非空 `absent title` 的空结果，以及名称排序下清空搜索后的完整卡片顺序。为避免重复，原有页面 trim/case/title-only/清空排序测试已由这一个三通道 Judge 替代；同文件保留独立的无结果 live status、空库、初始化/查询失败和刷新竞态测试。`video-title-query.test.ts` 保留 `%`/`_` 字面 SQL 和共享排序器的邻接合同。

实现经审计已符合合同，因此没有保留生产行为修改。首轮独立审查指出两个不扩展 AC 的缺口：Spec 与 Standards 均以 P1 要求双 adapter 同时裁判非空无匹配词；Standards 另以 P2 要求去除与新三通道 Judge 重复的页面 trim/case/title-only/清空排序测试。修复将 `absent title → []` 放入同一双 adapter 循环，并删除重复 Judge；保留独立的生产页面无结果 live status、空库、错误与竞态测试。为证明新增的双 adapter 非空无匹配断言并非自证，临时令 Memory `queryVideos` 对 `absent title` 关键词错误返回全部记录，定向运行真实 RED（6 个测试中 2 个失败：新增公共双 adapter Judge 与既有生产页面无结果 Judge），随后立即恢复原生产代码。最终定向验证 `npm.cmd exec -- vitest run src/__tests__/video-title-query.test.ts src/__tests__/video-list-title-search.test.tsx src/__tests__/video-list-sorting.test.tsx` 为 3 files / 12 tests GREEN；测试中预期的失败路径会输出 `console.error`。两名独立 reviewer 各自复跑该 3 files / 12 tests，Spec 与 Standards 复审均为 PASS，P0/P1/P2 为空；Standards 另复验 `tsc --noEmit`、`harness:control` 与 `git diff --check` 均 GREEN。实现阶段自行运行的 `npm.cmd exec -- tsc --noEmit`、`npm.cmd run harness:control`、普通 `npm.cmd run build`（只有既有动态/静态 import chunking warnings，production E2E isolation 通过）也均 GREEN。未改 locked `harness/` 或 `src-tauri/tests/`；本地实现/复审阶段未运行 `harness:check`、Cargo/Rust、Desktop E2E、安装包、GPU、模型、LLM 或 Evidence，且未手动 dispatch/rerun workflow。后续 commit、受保护 PR、自动 `Clean Windows Harness` 与合并由受控交付流程处理。已满足 Active Judge 后将 `AC-VL-03` 从 Partial 提升为 Strong；本 AC 不需要且未签发 Desktop、Visual 或 Evidence，也不外推到 `AC-VL-04`。

### 2026-08-29 Launch Slice — AC-SU-07 快捷键与焦点门禁

此未提交 Slice 位于 `codex/launch-su07-shortcut-focus-contract`，从精确 `origin/master` / `c593c3b872744e3eab8f8b4afc222b9764371d57` 推进，只收口 Confirmed `AC-SU-07` 与 `DEC-PRD-053`。生产 Owner 是 `StudyInterface` 内唯一挂载的 `src/study/shortcut-controller.ts`；其 media co-owner 是注入 controller 的 `src/study/media-session.ts`，负责 Study active-media registration 与 action interface，不反向依赖 UI video component。它们复用现有 Store `playPosition`/selection、`handleNodeNavigate` 的 TextPreview 同步和既有 Notes workflow。`App` 不再挂载旧全局 `ShortcutManager`，因此非 Study 页面没有第二个 `keydown` owner；锁定旧 M14 兼容组件保留且未改。此 Active 合同优先于旧 M14 的 Delete 语义：tree 或 diagram 选择下的 Del/Backspace 均只 `preventDefault`，不产生删除副作用。

未锁定 `src/__tests__/study-shortcuts.test.tsx` 是公开生产 Judge：真实内存数据库加载 StudyInterface、真实 `<video>`、Store、AI/随记 textarea、原生 input/contenteditable。它逐键锁定 `1/2/3` 对应 follow/textExpand/mapExpand（同档不回落 follow）、当前段反引号摘注、Space、±5/±10 seek、音量、N/P 选择/seek/TextPreview 和首末 no-op，以及 Tab 从非输入态先进入 AI、随后在 AI/随记间切换且将焦点送往目标输入。它还锁定全局键在所有输入/编辑态不被 preventDefault 或执行，ChatInput 的 Enter send 与 Alt+Enter newline 保持；非输入命令和 Del/Backspace 的 preventDefault 亦被观察。TDD 实际 RED 的新增单文件为 3 tests/3 failures，其中生产页面没有 owner，故 `1` 未 preventDefault 且反引号未创建真实 excerpt；最小 GREEN 后同文件为 3/3。另为证明输入门禁不是自证，临时移除 production editable-focus gate 后同一 Judge 真实 RED 为 2/3（唯一失败为输入态 `1` 被 preventDefault），随后立即恢复。

审计历史：前三轮独立审查依次发现 Tab target、zero-note composer/焦点、seek/volume 边界、媒体 seam、IME/Enter modifier、editable target、modifier 无副作用、single-flight/draft 保真和 pending 编辑丢失等 P1/P2。可由行为公开 Judge 裁判的缺口先形成真实 RED，再最小修正至 GREEN：初版 owner 缺失为 3/3 RED，移除 editable gate 为 2/3 RED；第二轮新增矩阵为 3/14 RED；第三轮 pending-edit 为 1/7 RED。controller 反向依赖 UI video component 则是结构审查发现：当时行为 Judge 为 GREEN，未虚称存在公开结构 RED；随后改为独立 `media-session` action/registration seam，并经独立复核。最终合同为：精准布局/媒体/N/P/Tab/Del-Backspace；AI、既有 Note 与 zero-note composer 门禁；zero-note composer 仅在显式保存时将未裁剪 draft 交给既有 workflow，pending 时 `readOnly` 且保存按钮 disabled，失败保留 draft。成功后由既有 workflow 发布 Note 并卸载 production composer；本 Slice 不声称保存后焦点必然落到持久 editor。

第四轮 Spec 与 Standards 独立复审均为 **PASS**，P0/P1/P2 均为 0；`AC-SU-07` 现为 **Strong**。最终 Judge 为 `study-shortcuts.test.tsx`（真实 StudyInterface、Store、video、面板与焦点/副作用矩阵）和 `study-notes.test.tsx`（真实 Notes workflow、single-flight、draft fidelity/pending edit），相邻 `study-playback`、`study-navigation`、`study-layout` 和只读 locked M08/M10/M14 同时回归。最终验证：Study + locked 只读集合 11 files / 78 tests GREEN；`tsc --noEmit`、普通 `build`（仅既有 Vite chunking warnings，E2E isolation 通过）、`harness:control`、`git diff --check` 均 GREEN。未改 `harness/` 或 `src-tauri/tests/`，未运行 `harness:check`、Cargo/Rust、Desktop E2E、安装包、GPU、模型/LLM、Release 或 workflow dispatch/rerun。`AC-SU-07` 不要求也未获得 Desktop、Visual、GPU、Release 或其他 Evidence，且不外推到 `AC-UX-06`、树/导图编辑或旧 M14 删除合同。

### 2026-08-31 Launch Slice — AC-SU-03 active assistant stream across right-panel Tabs

This uncommitted Slice is on `codex/launch-su03-stream-tabs`, created from exact `master@6ad944c80bea7fd03d2a07596806303f3fada4dc`. It narrows only the remaining local `AC-SU-03` assistant-session gap: a user may switch AI → 随记 while an existing assistant request is streaming without cancelling, restarting, or duplicating it. Tokens and normal completion remain attached to that original request while hidden; returning to AI reveals the same continuous answer. A returned active stream still uses the existing `停止` control, and a token emitted after that stop must not alter the answer. Out of scope: any new assistant persistence, Vision, Notes behavior, Tab shortcut policy, layout behavior, real LLM/Desktop Evidence, and all Rust/Release/GPU work.

Owner remains the existing `StudyInterface` assistant session state and its symmetric native-hidden Tab composition; no production implementation was retained because the audited behavior already met this bounded contract. The unlocked `src/__tests__/study-notes.test.tsx` Judge uses a real memory-backed Study load, real Store and StudyInterface, the public AI/随记 buttons and ChatInput, and only the public `streamAiChat` callback seam as the external-system substitute. It starts one request, emits a token, switches to Notes, emits its remaining token and completion while hidden, then returns to verify the original uninterrupted answer and no second request/cleanup. It starts a second request to prove the existing stop remains operable after a round trip and that a late callback token is ignored. The initial new Judge was GREEN because production already conformed; non-self-proving mutation RED then temporarily made the production AI wrapper always native-hidden. The same focused command became RED with the new active-stream Judge and the adjacent existing unsent-draft Judge both unable to access the visible AI input; the production line was restored immediately and the focused suite returned GREEN 8/8.

Allowed verification so far: the shared local dependency cache's `vitest run src/__tests__/study-notes.test.tsx` is GREEN 8/8. No locked `harness/` or `src-tauri/tests/` file changed, and no Cargo/Rust/Whisper/CUDA, `harness:check`, Desktop E2E, installer, GPU, model/real LLM, Evidence, workflow dispatch/rerun, commit, push, or PR operation occurred. `AC-SU-03` conservatively remains `Partial`: Strong plus required Desktop Evidence are still unsigned, and the remaining hidden-side-effect matrix is not claimed closed.

The first independent Spec/Standards reviews correctly rejected the shared cleanup spy: it could not prove that `停止` cleaned the second, current request rather than a stale first request. The corrective Judge returns a distinct cleanup spy for every public `streamAiChat` call, proves the completed first request stays at zero cleanup through the second request and its stop, requires only the second cleanup to be called once, and emits stale first `token`/`done` after the second request begins before proving that the second stream remains active and owns its visible answer. A temporary mutation that removed the production `handleStopMessage` cleanup made this Judge RED at 7/8 (`second cleanup expected 1, got 0`); it was immediately restored.

The second independent Spec/Standards reviews then found that the stale-first observation was delayed until the second token overwrote it. The Judge now immediately asserts after stale first `token`/`done`, before any second token, that the completed first answer is exactly unchanged, the second assistant message is still empty, and `停止` remains visible for the current stream. Temporarily weakening only the production `onToken` request-ID guard made the focused run RED at 7/8: the second message immediately became `First answer. stale first token`; that guard was restored. The corrected focused Judge and `tsc --noEmit` are GREEN; the required adjacent Study/M08/M10/M16 regression, ordinary build, control-plane and diff checks were re-run GREEN after this correction.

### 2026-08-31 PR #59 Clean Windows Harness failure record

Protected PR #59 head `41c08a2eee65a95b0d1d4bebd66ecad5e9d81b91` received automatic `Clean Windows Harness` run `33345606854` / job `99349009168`, which ended in `failure`. The clean Windows log shows `harness:control` passed; the frontend `npm test` stage passed 101 files with 1 skipped, including this Slice's `src/__tests__/study-notes.test.tsx` at 8/8; both `build:e2e` and ordinary production build completed and the production E2E-isolation scan passed. The only failing stage was Rust: `ytdlp::tests::cancellation_terminates_a_running_process_and_cleans_its_partial_output` panicked at unchanged `src-tauri/src/ytdlp_tests.rs:271` with `fixture must report progress: Elapsed(())`; the Rust summary was 97 passed, 1 failed.

This Slice's diff from `6ad944c80bea7fd03d2a07596806303f3fada4dc` changes only the unlocked Study test and control documents, with no `src-tauri/`, Rust Harness, workflow, package or build-script change. The same ytdlp fixture source is byte-identical to that baseline. Its preceding PR #58 Harness run `33299368098` succeeded at `04bd1781e87bdcbbd322286de6768f3f010d0d74`, and master-push Harness run `33299772643` succeeded at the exact baseline `6ad944c80bea7fd03d2a07596806303f3fada4dc`. The failure is therefore recorded as an adjudication failure unrelated to the current Slice; its cause is not established and it is not called a flake. No local Cargo/Rust run was performed and no Rust or locked Harness file was modified. The next normal PR push will obtain the automatic independent Harness judgment; no manual rerun or dispatch is authorized by this record.

### 2026-08-31 PR #59 protected merge and post-merge Harness record

PR #59's replacement automatic `Clean Windows Harness` run `33346306370` / job `99351003602` succeeded for PR head `925742b5c59c459a071584c25673f92a8bd1e2a9`. The protected PR was then merged at `2026-08-31T01:13:17Z` as `master@02f138d4a5067554d464b5b92cb5daf75d06c6c1`. This closes the PR's required automatic pre-merge Harness judgment; `AC-SU-03` remains `Partial` and this record does not change coverage, audit status, Strong/desktop requirements, or any product claim.

The automatic master-push `Clean Windows Harness` run `33347032199` / job `99353002696` for that exact merge commit ended in `failure`. Its frontend summary is 3 failed, 746 passed and 1 skipped. The three failing tests are outside the merged Slice and unchanged by its diff: `scripts/nvidia-release-evidence.test.ts` timed out at 15 seconds in the canonical whisper-backends provenance case, and its PowerShell cleanup additionally reported child processes rejecting `SIGKILL`; `scripts/release-artifact-generator.test.ts` timed out at 5 seconds in each of its pinned-download-hash and explicit Blackwell-compatible CUDA-architecture rejection cases. No `scripts/`, Rust, workflow, source implementation or locked Harness file was changed by the Slice (its tracked diff from the preceding master contains only the unlocked Study test and control documents). The root cause is not established and these failures are not called flakes. No local Cargo/Rust or prohibited command was run for this record; future normal protected changes obtain their usual automatic Harness judgment without manual dispatch or rerun.
