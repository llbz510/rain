...
The no-key Runtime Settings desktop behavior Judge is locally proven, but `AC-HE-05` remains a current Gap because the manual Hosted Windows Judge is still RED. Workflow_dispatch run `30339238218` proved the clean build and exact WebView2/driver pair, then failed at first Tauri session creation because the process environment's browser arguments had not been injected into the Tauri window context. Workflow YAML, local GREEN or the default Harness cannot substitute for a successful hosted target-commit result.
...
18. `AC-HE-05` is Confirmed but not yet Strong. The workflow is present on the default branch and real Hosted Windows REDs are recorded, most recently run `30339238218` at Tauri session creation. Keep the Gap until a target-commit workflow_dispatch completes the real desktop command; do not add automatic PR/push triggers or claim local/default-Harness GREEN as a substitute.
