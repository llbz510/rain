...
| `npm run evidence:nvidia-release -- --InstallerPath <path> --ArtifactManifestPath <release-artifact-manifest.json> --ExpectedArtifactManifestSha256 <sha256-from-controlled-merged-target-build-record> --WhisperModelPath <path> --ExpectedTargetCommit <sha> --ExpectedInstallerSha256 <sha256>` | M3-S3 runner 的受控接口；启动安装器前先以独立 expected manifest SHA-256 核对受控 merged-target build record、目标提交和 installer 字节。确定性 TEMP cancellation fixture 与 session-scoped process-start adapter 的本地 enablement 已实现，runner 不再固定阻断于 `runtime-adapter-readiness`；fixture 创建/释放/删除、callback 时间窗、订阅/job/provider 健康或 Rain 进程归属任何不可信都会 fail-closed。尚未在当前目标 installer/GPU/model 上真实执行，不能据此签发 Evidence 或升级 `AC-RL-08`；不调用 LLM |
