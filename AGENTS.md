...
`docs/PROJECT_STATE.md` 是简洁的当前项目状态快照，记录：

- 当前可验证状态；
- 已知缺陷；
- 当前 Slice 的短期交接与维护规则。

它不保存无限追加的会话日志；已合并改动和历史验证应回到 Git/PR 历史读取。

如果本轮会话修改了项目文件，交付前必须替换式地同步更新 `docs/PROJECT_STATE.md` 的当前事实和当前交接，不能新增按日期的历史段落。
...
| `npm run build:whisper-gpu-worker` | 构建并暂存隔离 CUDA worker 与所需 CUDA runtime（不包含驱动提供的 `nvcuda.dll`） |
| `npm run bundle:gpu` | 构建默认 `Auto` 的 GPU 产品包；Rain 主程序仍为 CPU-safe，CUDA 仅在资源 worker 中 |
| `npm run evidence:nvidia-release -- --InstallerPath <path> --ArtifactManifestPath <release-artifact-manifest.json> --ControlledBuildRecordPath <controlled-build-record.json> --ExpectedArtifactManifestSha256 <sha256-from-controlled-merged-target-build-record> --WhisperModelPath <path> --ExpectedTargetCommit <sha> --ExpectedInstallerSha256 <sha256>` | M3-S3 runner 的受控接口；启动安装器前先以独立 expected manifest SHA-256 和显式 controlled-build record 核对受控 merged-target build record、目标提交和 installer 字节。确定性 TEMP cancellation fixture 与 session-scoped process-start adapter 的本地 enablement 已实现，runner 不再固定阻断于 `runtime-adapter-readiness`；fixture 创建/释放/删除、callback 时间窗、订阅/job/provider 健康或 Rain 进程归属任何不可信都会 fail-closed。尚未在当前目标 installer/GPU/model 上真实执行，不能据此签发 Evidence 或升级 `AC-RL-08`；不调用 LLM |
